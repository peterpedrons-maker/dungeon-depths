...
**v2 — reescrito depois que a v1 saiu simples demais.** O pedido original resultou numa peça basicamente lisa (superfície e textura, mas sem relevo de verdade); o que se quer é o oposto — uma moldura no estilo HUD de MMO (World of Warcraft, Diablo) onde a PRÓPRIA moldura é coberta de entalhe/relevo real: filigrana, cabeças esculpidas, gemas encravadas, ferragem — não um container liso com um gradiente por cima. O prompt abaixo pede isso explicitamente, com uma descrição bem mais densa do ornamento.

```
Hand-painted medieval fantasy MMO UI asset, rich digital painting style — think World of Warcraft's ornate top HUD frame, Diablo's carved stone-and-gold panels, and Baldur's Gate 3's engraved metalwork. This is NOT a flat textured surface — it is a densely carved, heavily ornamented relief sculpture in dark iron and aged gold, with real dimensional depth. Every part of the frame shows hand-carved detail; nowhere is it just empty texture.

A wide horizontal banner spanning the full canvas, front-on view, no perspective. The base material is dark blackened iron and dark stone (warm charcoal-brown: near #443a33 fading to #2b2013), but its entire surface is covered in raised, sculpted ornamentation: a continuous engraved vine-and-thorn scrollwork pattern runs along the top edge and the bottom edge like a carved cornice, catching strong warm highlights on its raised ridges and deep shadow in its grooves.
...
Between the two end-clusters, the broad middle two-thirds of the banner is where the game overlays the hero's name, level, gold/potion counts, a settings gear, and two horizontal status bars — keep the carving in this zone shallower and its palette darker/lower-contrast than the ends so light-colored text and icons stay legible on top of it, but it must still read as part of the same continuous carved plaque, never a plain empty gap: a subtle repeating engraved diamond-and-rune motif runs the whole width just inside the top and bottom cornices, and one fine inset groove line runs horizontally roughly 60% of the way down the banner's height, like a carved ledge separating an upper register (hero info) from a lower one (status bars).
