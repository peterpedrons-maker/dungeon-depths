...
> Substitui o prompt antigo — a classe foi redesenhada depois que esse prompt foi escrito e a arte já entregue (`feiticeiro.webp`) ficou obsoleta: nem as trilhas ("Explosão Arcana"/"Sobrecarga Mística"/"Domínio Arcano") nem nenhum dos 15 nomes de habilidade batem com a árvore real de hoje (Ruptura/Reverberação/Moldagem). Reescrito do zero com os nomes e cores reais de `skills.ts`, um nó por vez, contra a descrição mecânica real de cada um em `skills.ts` (não só o nome).
>
> **v2:** trocado "bolt voando" por cristal/onda/runa — usuário achou pior, virou diagrama técnico parado.
>
> **v3:** trocado por cena de ação dramática (mãos conjurando, explosão) — usuário apontou dois problemas novos: (1) nenhum ícone tinha a ver com a arma real da classe (grimório — conferido de novo contra `weapon-feiticeiro.webp`, 10 variações, todas livro/tomo, nunca cajado/varinha/cristal solto); (2) ícones entre si (e entre Ativas e Passivas) ficaram parecidos demais.
>
> **v4 (esta versão) — reescrita nó a nó, devagar, uma classe por vez a pedido do usuário:** o grimório agora é o objeto-âncora physically presente em TODOS os 24 ícones da classe (15 Ativas + 9 Passivas), mas nunca do mesmo jeito duas vezes — cada ícone mostra uma parte/estado diferente do livro (página virando, capa fechada, lombada, marcador, canto queimando, as duas páginas abertas) interagindo com o efeito daquele nó específico. Pra evitar repetição Ativas×Passivas, as duas folhas têm registros visuais deliberadamente diferentes: **Ativas = o livro em pleno feitiço**, ação dinâmica no auge (página explodindo, luz jorrando) — **Passivas = o livro em repouso, com a marca permanente do efeito** (brasa gravada na capa, elo na lombada, correção na margem), refletindo que passivas são bônus constantes, não uma ação instantânea. Cada um dos 24 ícones foi desenhado individualmente contra a descrição mecânica exata do nó (não reaproveitando composição de nenhum outro).

```
Hand-painted medieval fantasy game UI asset, rich digital painting style, icon set for a talent/ability tree — same painted style as the game's other UI icons (gold coin, health potion, ruby heart), NOT flat vector, NOT pixel art. Every icon must read the way a real, professionally-painted RPG/MOBA ability icon reads: a bold, dramatic, high-impact action caught at its single most striking instant, with a strong, instantly-readable silhouette that stays legible even shrunk down very small. NEVER a flat technical diagram, NEVER a calm static symbol or glyph just floating alone with no object grounding it — always a scene of raw power happening right now, rendered with dramatic directional lighting, deep shadow, and a bright glowing core light source.
...
IMPORTANT GLOBAL RULES: (1) A weathered leather-bound grimoire — dark brown leather cover, a brass clasp, aged cream-colored pages with faint handwritten script — must appear, at least partially, in every one of the fifteen medallions below; it is this character's only weapon, always the visible source of every effect. (2) No two of the fifteen medallions may show the grimoire in the same pose or state (open flat, open with one page lifted, snapping shut, fully splayed with pages fanned, etc. — each icon's exact book-state is specified below; follow it precisely so no two icons end up looking alike). (3) None of the fifteen icons may depict a flying projectile, arrow, bolt, or dart of any kind. (4) No icon may show a floating rune/glyph/symbol with no grimoire underneath or behind it grounding where the magic is coming from.

Row 1 ("Ruptura" path — every medallion's rim and inner glow painted in a violet-magenta, #a03fb8; the grimoire's magic here is raw explosive pressure, cracking things open): 1) "Impacto de Origem" — the grimoire lies open flat, facing the viewer; its left page burns with one small violet sigil, a compact burst of violet light erupting straight up off that single page's surface; the right page stays plain cream-colored paper, unlit. 2) "Compressão Violenta" — the grimoire's two open pages are caught at the exact instant of slamming toward each other to close, a blinding violet light and jagged crack-lines bursting out from the narrowing gap between the pages just before they meet. 3) "Ruptura Gêmea" — the grimoire lies open flat with both pages facing the viewer; each page burns with its own identical violet sigil at the same instant, twin symmetrical bursts of violet light erupting upward off both pages together. 4) "Ponto de Colapso" — the grimoire's pages are shown mid-tear, several loose violet-glowing page-fragments ripped free and spiraling inward, all being sucked toward one small blinding point of violet light hovering just above the book's open spine. 5) "Supernova Interior" — the grimoire is caught at the exact instant its front cover is blown wide open by a catastrophic internal detonation, blazing violet paper-shard fragments and rays of light bursting outward from between its pages in every direction, filling nearly the whole medallion.

Row 2 ("Reverberação" path — every medallion's rim and inner glow painted in a warm gold, #c89a2e; the grimoire's magic here is sound-like resonance, pages ringing and echoing): 6) "Pulso Bifurcado" — the grimoire lies open flat; one golden rune-glyph rises off each of its two open pages at the same instant, and each glyph is immediately trailed by its own smaller, fainter echo-glyph hovering just behind and offset from it — four glyphs total, two bright and two faint. 7) "Eco Instável" — the grimoire lies open with only its left page lifted slightly off the right, as if caught mid-turn; one golden rune-glyph blazes brightly above the lifted page, while a flickering, visibly distorted echo-copy of that same glyph hovers unstably just beside it, its edges crackling like static. 8) "Cascata Arcana" — the grimoire's pages are caught mid-flip in a rapid blurred fan (three overlapping translucent page-edges suggesting motion), one small golden burst of light escaping from beneath each of the three blurred pages in a cascading diagonal line, each burst smaller than the last. 9) "Recorrência" — the grimoire lies open flat with one golden rune-glyph glowing above its right page; a small golden hourglass rests directly on the open page beside the glyph, its sand visibly flowing upward against gravity. 10) "Sobrecarga Viva" — the grimoire is shown fully splayed open, its pages fanned wide in an overloaded frenzy; four golden rune-glyphs blaze simultaneously above four different fanned pages, crackling arcs of golden energy leaping wildly between them.

Row 3 ("Moldagem" path — every medallion's rim and inner glow painted in a burnt orange-red, #c1502e; the grimoire's magic here is precise geometric diagrams, shaping and cutting with exact control): 11) "Vetor Forçado" — the grimoire lies open flat to a page bearing one hand-drawn orange-red geometric diagram (straight ruled lines converging to a single focal point); a thin, perfectly straight beam of orange-red light snaps taut directly out of that diagram's focal point, standing rigid above the page. 12) "Campo de Distorção" — the grimoire lies open to a page bearing one glowing orange-red circular ward-diagram; the air directly above that page visibly ripples and warps outward from the diagram like heat-shimmer, bending the space only in that zone above the open book. 13) "Corte de Realidade" — a single blade-thin beam of orange-red light lances straight up out of the grimoire's open page, its edge having just sliced clean through one small translucent barrier-panel hovering just above the book, the panel split open in one straight line exactly along the beam's path. 14) "Forma Impossível" — the grimoire lies open to a page covered in three overlapping orange-red geometric diagrams, their straight-edged lines visibly rotating and realigning over each other above the page, the third and final diagram glowing brighter than the other two as it locks into place. 15) "Forma Absoluta" — the grimoire lies fully open, both pages covered by one single, complete, fully-assembled orange-red geometric diagram spanning across the entire two-page spread, blazing at maximum brightness and tearing a jagged glowing rift open in the air directly above the open book.
...
> **v4** (mesma revisão da folha de Ativas acima, ver nota lá) — registro visual deliberadamente diferente das Ativas: lá o grimório está em pleno feitiço (página explodindo, luz jorrando); aqui ele está **em repouso**, mostrando a marca PERMANENTE que a passiva deixou nele — brasa gravada na capa, estudo entalhado na lombada, correção anotada na margem — porque uma passiva é um bônus constante, não uma ação instantânea. O grimório também aparece nos nove ícones abaixo, cada um num estado diferente.

```
Hand-painted medieval fantasy game UI asset, rich digital painting style, icon set for a talent/ability tree — same painted style as the game's other UI icons (gold coin, health potion, ruby heart), NOT flat vector, NOT pixel art. Every icon must read the way a real, professionally-painted RPG item/ability icon reads: a richly rendered object caught in a specific, telling moment, with a strong, instantly-readable silhouette that stays legible even shrunk down very small. NEVER a flat technical diagram with arrows or callouts, NEVER a symbol floating alone with nothing grounding it.
...
IMPORTANT GLOBAL RULES: (1) The same weathered leather-bound grimoire from the Ativas sheet — dark brown leather cover, brass clasp, aged cream-colored pages — appears, at least partially, in every one of the nine medallions below, always shown closed, resting, or otherwise NOT mid-cast (this sheet is about the book carrying a permanent mark, not casting a spell right now). (2) No two of the nine medallions may show the grimoire in the same pose or in the same part of the book (cover, spine, clasp, page-edge, corner — see each icon's exact instruction below). (3) None of the nine icons may depict a flying projectile, arrow, bolt, or dart. (4) No icon may show a floating rune/glyph with no part of the grimoire visibly underneath or behind it.

Row 1 ("Ruptura" path — every medallion's rim and inner glow painted in a violet-magenta, #a03fb8): 1) "Fratura Arcana" — the grimoire's closed front cover, facing the viewer; three small crack-shaped brand-marks are burned into the leather near its brass clasp in a short row, only the first mark glowing bright violet, the other two still cold dark outlines. 2) "Rachadura Espontânea" — the grimoire's closed cover, caught at the exact instant one small violet ember lands on the leather and burns a single sharp crack-shaped mark into it, a tiny wisp of smoke curling up from the fresh mark. 3) "Depois da Explosão" — the grimoire's closed cover shows the same row of three crack-shaped brand-marks as icon 1, but now the first two have faded to cold grey ash-colored scars while only the third, rightmost mark still glows warm violet.

Row 2 ("Reverberação" path — every medallion's rim and inner glow painted in a warm gold, #c89a2e): 4) "Ressonância" — the grimoire's spine, standing upright, with two small round golden studs embedded side by side along the binding, both glowing at matched steady brightness. 5) "Segunda Onda" — a golden ribbon bookmark hanging down from between the grimoire's closed pages, its tip marked with a small crit-starburst; a second, fainter, translucent echo-ribbon overlaps directly behind the solid one, offset just slightly. 6) "Perpetuum Arcano" — the grimoire's pages pressed shut, seen edge-on from the fore-edge; thin lines of golden light are visibly leaking out from between the pressed pages, with a few bright droplet-like flares escaping past the book's outer edge.

Row 3 ("Moldagem" path — every medallion's rim and inner glow painted in a burnt orange-red, #c1502e): 7) "Controle Arcano" — the grimoire's closed cover, a small precise orange-red compass-and-straightedge diagram tooled directly into the leather at its center, sharp clean lines catching a bright specular highlight. 8) "Correção Instantânea" — the same tooled orange-red diagram from icon 7 on the grimoire's cover, with one small bright checkmark-shaped correction-glyph freshly burned into one corner of it, overlapping and fixing part of the original design. 9) "Lei Pessoal" — the grimoire's spine, a thin unbroken loop of orange-red light traced along the binding from top to bottom, its glowing line curving fully around the spine's base and closing back into its own starting point at the top.
