...
> Mesmo método v5/v2 da folha de Ativas acima: **nenhum ícone mostra o bardo nem o alaúde** — só o fenômeno musical puro, num estado mais contido/latente/permanente do que o auge das Ativas.
>
> **v2 (mesmo feedback da folha de Ativas):** a v1 usava "nota musical" como objeto-base em 6 dos 9 ícones (só variando o que acontecia com ela) — mesma falha das moedas do Bruxo. Reescrito: nenhum ícone abaixo usa uma nota musical; cada um dos 9 tem um objeto físico totalmente diferente (frasco, pêndulo, sino, carta, ponte, máscara etc.).

```
Hand-painted medieval fantasy game UI asset, rich digital painting style, icon set for a talent/ability tree — same painted style as the game's other UI icons (gold coin, health potion, ruby heart), NOT flat vector, NOT pixel art. Every icon must read the way a real, professionally-painted RPG item/ability icon reads: a richly rendered musical phenomenon with a strong, instantly-readable silhouette that stays legible even shrunk down very small. NEVER a flat technical diagram with arrows or callouts, NEVER a symbol floating with no visual weight or context.

Each of the nine icons is a CIRCULAR MEDALLION (a coin-like RPG skill-icon badge, not a loose picture floating on the background): a dark, almost-black disc with a soft glow radiating from its center in that row's theme color, ringed by a thick ornamented metal border with small triangular points/teeth jutting outward around the rim, painted in that same row's theme color. Inside the disc, the scene fills about 65-70% of the disc's inner area — richly rendered with warm directional lighting from the upper-left and a bright specular highlight. NO text, name, number, or label anywhere on or under any medallion.

One single image containing NINE separate medallions arranged in an even 3-column × 3-row grid, generous magenta gaps between every medallion so each can be cropped out individually later. Every medallion the same size, centered in its own cell, same painted lighting style and rendering quality across all nine — each row's medallions share the exact same rim/glow color, and no two rows share a color.

IMPORTANT GLOBAL RULES: (1) No bard, hand, silhouette, or lute anywhere in this sheet — every icon shows only the pure musical phenomenon itself, floating in dark space. (2) Every icon should feel contained and latent rather than mid-explosion — a held beat, a steady glow, a resting note — never the peak-intensity action of the Ativas sheet. (3) None of the nine icons may depict a flying projectile, arrow, bolt, or dart. (4) NO musical-note glyph shape may appear anywhere on this sheet — every icon below uses a distinct physical object instead (a vial, a pendulum, a bell, a card, a bridge, a mask, etc.); no two icons anywhere on this sheet may share the same central object.

Row 1 ("Marcha de Guerra" path — every medallion's rim and inner glow painted in a rust-orange, #c9663c): 1) "Batida Marcada" — a single accent-stress rhythm mark (a small ">" wedge shape, not a note) glowing steady rust-orange, floating alone in dark space with no other elements around it. 2) "Sem Perder o Ritmo" — a single broken drumstick, snapped cleanly in half, its splintered tip still glowing with a faint rust-orange ember of rhythm. 3) "O Palco é Meu" — a narrow cone of rust-orange spotlight shining down onto one small empty circle marked on a dark stage floor.

Row 2 ("Dissonância" path — every medallion's rim and inner glow painted in a dark violet, #4a2a5a): 4) "Eco Roubado" — a small cracked crystal vial, half-filled with glowing dark violet light and half empty, as though its other half had been drained away and stolen. 5) "Contratempo Perfeito" — a metronome's pendulum arm frozen mid-swing, tilted at an unnatural backward angle away from the straight vertical resting-line marked behind it, both glowing dark violet. 6) "O Som Depois do Som" — a small hand bell with two sound-ripples still radiating outward from a single strike, the first ripple bright and close, the second trailing ripple noticeably fainter and farther out.

Row 3 ("Improviso" path — every medallion's rim and inner glow painted in a light gold, #e0c060): 7) "Verso Livre" — a small rectangular tile marked with a jester's pointed three-tipped cap silhouette (a wildcard token, not a note), glowing warm gold, floating alone in dark space. 8) "Ponte Musical" — a short wooden plank bridge spanning a narrow gap between two bare stone ledges, a line of golden light traveling steadily across the plank's length. 9) "O Espetáculo Continua" — a golden theatrical comedy-mask shape, rendered faint and translucent like an afterimage, a single wisp of light trailing from one corner of its painted smile.
