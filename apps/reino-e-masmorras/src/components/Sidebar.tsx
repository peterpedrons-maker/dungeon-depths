...
export function Sidebar({ section, onNavigate, onEnterDungeon, onAbandon }: Props) {
  return (
    <nav className="w-56 shrink-0 bg-panel border-r-2 border-panelborder flex flex-col">
...
function Group({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border-b border-panelborder/60">
      <div className="px-3 py-1.5 bg-panel2 text-gold/80 text-xs font-bold uppercase tracking-wider">{title}</div>
