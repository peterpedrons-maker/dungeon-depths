...
const warriorAtkScale = (posture: string): ScalingEntry[] => [
  { attribute: 'str', label: 'FOR', role: 'principal', description: 'Aumenta o ATK usado por este golpe.' },
...
export const SKILL_TREES: Record<ClassId, SkillPath[]> = {
  guerreiro: [
    buildPath('guerreiro', 'furioso', 'Vanguarda', '#a5432f', [
      { name: 'Ombro à Frente', desc: '+2% de dano físico direto. O primeiro golpe de uma habilidade de Vanguarda que acertar cada inimigo causa +6 de Dano de Postura.', effect: { dmgPct: 0.02 }, mechanicRefs: ['guerreiro:posture'], scaling: warriorFixedScale('O bônus de Postura é fixo e ocorre uma vez por inimigo.') },
      ...
      { name: 'Fôlego de Vanguarda', desc: '+8 de Vida Máxima. Com Postura inimiga em 33 ou menos, recebe 3% menos dano direto.', effect: { maxHpFlat: 8 }, mechanicRefs: ['guerreiro:posture'], scaling: warriorVitScale('VIT aumenta a Vida Máxima; a redução de dano condicional não escala com atributos.') },
      { name: 'Ritmo de Assalto', desc: '-3% de Recarga somente para habilidades de Vanguarda. Ao acertar uma delas, o próximo ataque básico causa +2 de Dano de Postura.', effect: {}, mechanicRefs: ['guerreiro:posture'], scaling: warriorFixedScale('Recarga e Postura extra são valores fixos.') },
      { name: 'Quebra-Linha', desc: 'Cause 1,45x ATK e 18 de Dano de Postura. Contra Postura Firme, causa 22 de Postura. Recarga: 3 ciclos.', ability: { name: 'Quebra-Linha', desc: '1,45x ATK e 18 de Postura; 22 contra Postura Firme.', cooldown: 3, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.45, postureDamage: 18, postureDamageFirm: 22, vanguardAbility: true } }, mechanicRefs: ['guerreiro:posture'], scaling: warriorAtkScale('O Dano de Postura é fixo, maior apenas se o alvo já estava em Postura Firme.') },
      ...
      { name: 'Golpe de Pressão', desc: 'Cause 1,60x ATK e 16 de Dano de Postura. Ao acertar, aplica Suprimido por 2 ações reais: recuperação máxima de 4. Recarga: 4 ciclos.', ability: { name: 'Golpe de Pressão', desc: '1,60x ATK, 16 de Postura e Suprimido por 2 ações reais.', cooldown: 4, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.60, postureDamage: 16, suppressPostureRecoveryActions: 2, vanguardAbility: true } }, mechanicRefs: ['guerreiro:posture'], scaling: warriorAtkScale('O Dano de Postura é fixo; o efeito Suprimido não acumula.') },
      { name: 'Aríete', desc: 'Dois impactos independentes de 0,75x ATK e 9 de Postura cada. Se ambos acertarem, a próxima recuperação natural é 0. Recarga: 5 ciclos.', ability: { name: 'Aríete', desc: '2 impactos de 0,75x ATK e 9 de Postura; ambos acertando zeram a próxima recuperação.', cooldown: 5, condition: { type: 'always' }, effect: { kind: 'multiHit', hitCount: 2, dmgMultPerHit: 0.75, postureDamagePerHit: 9, zeroNextPostureRecoveryIfAllHits: true, vanguardAbility: true } }, mechanicRefs: ['guerreiro:posture'], scaling: warriorAtkScale('O Dano de Postura por impacto é fixo.') },
      { name: 'Penetração Marcial', desc: 'Contra um inimigo com Postura baixa, ataques físicos diretos recebem penetração de DEF extra.', effect: {}, mechanicRefs: ['guerreiro:posture'], scaling: warriorFixedScale('Penetração condicional fixa; soma com Guarda Quebrada dentro dos caps.') },
      { name: 'Romper Guarda', desc: 'Requer Postura em 30 ou menos. Cause 2,10x ATK e 30 de Dano de Postura. Se quebrar a Guarda, a janela recebe +1 ação ofensiva, até 3. Recarga: 6 ciclos.', ability: { name: 'Romper Guarda', desc: '2,10x ATK e 30 de Postura; a quebra pode durar 3 ações ofensivas.', cooldown: 6, condition: { type: 'enemyPostureAtMost', value: 30 }, effect: { kind: 'bigHit', dmgMult: 2.10, postureDamage: 30, guardBreakActionsBonusOnBreak: 1, vanguardAbility: true } }, mechanicRefs: ['guerreiro:posture', 'guerreiro:guardbreak'], scaling: warriorAtkScale('O Dano de Postura é fixo.') },
      { name: 'Avanço Irrefreável', desc: 'Três impactos independentes de 0,65x ATK e 7 de Postura cada. Uma quebra entre impactos fortalece imediatamente os restantes. Recarga: 8 ciclos.', ability: { name: 'Avanço Irrefreável', desc: '3 impactos de 0,65x ATK e 7 de Postura, com quebra imediata entre hits.', cooldown: 8, condition: { type: 'always' }, effect: { kind: 'multiHit', hitCount: 3, dmgMultPerHit: 0.65, postureDamagePerHit: 7, vanguardAbility: true } }, mechanicRefs: ['guerreiro:posture', 'guerreiro:guardbreak'], scaling: warriorAtkScale('O Dano de Postura por impacto é fixo.') },
      { name: 'Domínio da Linha', desc: 'Quando Guarda Quebrada termina, a Postura do inimigo volta a um valor menor do que o normal.', effect: {}, mechanicRefs: ['guerreiro:guardbreak'], scaling: warriorFixedScale('O valor de retorno é fixo e não acumula.') },
    ]),
    buildPath('guerreiro', 'guardiao', 'Mestre da Guarda', '#6b7280', [
      ...
      { name: 'Riposta Imediata', desc: 'Todo Aparo bem-sucedido prepara uma Riposta: a próxima habilidade ofensiva recebe +0,18x ATK e +6 de Postura.', effect: {}, mechanicRefs: ['guerreiro:riposte'], scaling: warriorAtkScale('O Dano de Postura da Riposta é fixo.') },
      { name: 'Técnica de Impacto', desc: 'VIT melhora a redução do Aparo em 0,15 ponto percentual por VIT total, até +8 pontos percentuais.', effect: {}, mechanicRefs: ['guerreiro:parry'], scaling: warriorVitScale('Cada ponto de VIT amplia diretamente a redução do Aparo, dentro de um teto próprio.') },
      ...
      { name: 'Contra-Golpe Perfeito', desc: 'Requer Riposta e Postura inimiga em 40 ou menos. Cause 2,35x ATK e 26 de Postura. Se quebrar a Guarda, o primeiro ataque da janela recebe +5 pp de Precisão. Recarga: 8 ciclos.', ability: { name: 'Contra-Golpe Perfeito', desc: 'Requer Riposta e Postura até 40; 2,35x ATK e 26 de Postura.', cooldown: 8, condition: { type: 'all', conditions: [{ type: 'riposteReady' }, { type: 'enemyPostureAtMost', value: 40 }] }, effect: { kind: 'bigHit', dmgMult: 2.35, postureDamage: 26, riposteRequired: true, perfectCounterAccuracyOnBreak: true, guardAbility: true } }, mechanicRefs: ['guerreiro:riposte', 'guerreiro:guardbreak'], scaling: warriorAtkScale('O Dano de Postura antes da Riposta é fixo.') },
      { name: 'Mestre do Aparo', desc: 'Todos os Aparos recebem +5 pontos percentuais de redução. Se um Aparo causar Guarda Quebrada, a janela recebe +1 ação ofensiva, até 3.', effect: {}, mechanicRefs: ['guerreiro:parry', 'guerreiro:guardbreak'], scaling: warriorVitScale('Aplica depois do valor base e antes do teto final de redução do Aparo.') },
    ]),
    buildPath('guerreiro', 'duelista', 'Duelista', '#c89a2e', [
      { name: 'Olho de Espadachim', desc: '+2 pontos percentuais de Precisão. O primeiro golpe de habilidade Duelista que acertar cada inimigo enquanto Firme causa +5 de Postura.', effect: { accuracyPct: 0.02 }, mechanicRefs: ['guerreiro:posture'], scaling: warriorFixedScale('Precisão e o bônus de Postura são fixos.') },
      ...
      { name: 'Destreza Marcial', desc: 'DES aumenta o Dano de Postura das habilidades Duelistas em 0,30% por DES total, até +12%.', effect: {}, mechanicRefs: ['guerreiro:posture'], scaling: warriorDexScale('Cada ponto de DES amplia diretamente o Dano de Postura das habilidades Duelistas, dentro de um teto próprio.') },
  ...
  // Legacy pre-redesign tree retained only in source history; the live
  // table is assigned declaratively below (SKILL_TREES.ladino = [...]),
  // same convention as necromante/bardo/druida.
  ladino: [],
  ...
  cavaleiro: [
    ...
    buildPath('cavaleiro', 'comando', 'Comando', '#e0c060', [
      ...
      { name: 'Estandarte do Rei', desc: 'Habilidade ativa: por 4 ciclos, +10% de ATK, +12% de defesa e +10 pontos percentuais de Tenacidade (escalados diretamente por SAB). Gera +1 Ordem ao ativar. A primeira outra habilidade de Comando usada durante o efeito reembolsa +1 Ordem (uma vez). Recarga de 15 ciclos.',
        mechanicRefs: ['cavaleiro:orders'],
        ability: {
          name: 'Estandarte do Rei', desc: 'Por 4 ciclos: +10% ATK, +12% DEF, +10 pontos percentuais de Tenacidade. Gera +1 Ordem; a primeira outra habilidade de Comando usada reembolsa +1 Ordem.',
  ...
  // Legacy pre-redesign tree retained only in source history; the live
  // table is assigned declaratively below (SKILL_TREES.paladino = [...]),
  // same convention as necromante/bardo/druida.
  paladino: [],
  ...
  arqueiro: [
    buildPath('arqueiro', 'precisao', 'Arco Pleno', '#c89a2e', [
      { name: 'Mão Estável', desc: '+1,5 ponto percentual de Precisão. Com 50 ou mais de Tensão, DES concede até +2 pontos percentuais adicionais.', effect: { accuracyPct: 0.015 }, mechanicRefs: ['arqueiro:distance','arqueiro:tension'] },
      { name: 'Pulso Frio', desc: '+1 ponto percentual de Crítico. Com 75 ou mais de Tensão, SOR concede até +2 pontos percentuais adicionais.', effect: { critPct: 0.01 }, mechanicRefs: ['arqueiro:tension'] },
      { name: 'Força de Puxada', desc: '+2% de dano físico. Disparos Precisos usados com 75 ou mais de Tensão recebem até +3% adicionais por DES.', effect: { dmgPct: 0.02 }, mechanicRefs: ['arqueiro:tension'] },
      ...
      { name: 'Tiro de Medição', desc: 'Disparo Preciso de 1,30x ATK que gera 55 de Tensão (70 se você já estiver no Horizonte) e faz sua distância retornar ao Horizonte. Um erro faz perder 8 de Tensão.', ability: { name: 'Tiro de Medição', desc: '1,30x ATK; gera 55 de Tensão (70 no Horizonte) e retorna ao Horizonte. Um erro faz perder 8 de Tensão.', cooldown: 3, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.30, archerPath: 'precision', archerShotType: 'precise', archerTensionOverrideOnHit: 55, archerTensionOverrideAtHorizon: 70, archerDistanceShift: 3 } }, mechanicRefs: ['arqueiro:distance','arqueiro:tension'] },
      { name: 'Visão do Horizonte', desc: '+4% de Dano Crítico. No Horizonte, Disparos Precisos e Flechas em Voo recebem +2% de dano final adicional.', effect: { critDmgPct: 0.04 }, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Firmeza do Arco', desc: 'Com 50 ou mais de Tensão, Disparos Precisos recebem +4% de dano; com Tensão em 100, o bônus total sobe para +8%.', effect: {}, mechanicRefs: ['arqueiro:tension'] },
      { name: 'Respiração Longa', desc: '+1,5 ponto percentual de Evasão. Em distância 2 ou mais, AGI concede até +2 pontos percentuais adicionais.', effect: { evasionPct: 0.015 }, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Recuo Calculado', desc: 'A cada 4 ações, um Disparo Preciso que acertar em distância 0 ou 1 avança sua distância em 1, até um máximo acumulado de 2.', effect: {}, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Flecha Perfurante', desc: 'Requer 25 de Tensão, consumida no início do cast. Causa 1,70x ATK com 15% de penetração de DEF; se você tinha 75 ou mais de Tensão no início, 1,85x e 18% de penetração.', ability: { name: 'Flecha Perfurante', desc: '1,70x ATK e 15% de penetração de DEF (1,85x e 18% com 75 ou mais de Tensão no início).', cooldown: 4, condition: { type: 'resourceAtLeast', resource: 'tension', value: 25 }, effect: { kind: 'bigHit', dmgMult: 1.70, archerPath: 'precision', archerShotType: 'precise', archerRequiresTension: 25, archerTensionCost: 25, archerHighTensionDmgMult: 1.85, archerHighTensionPenPct: 0.18, archerDefPenPct: 0.15 } }, mechanicRefs: ['arqueiro:tension'] },
      { name: 'Flecha Balística', desc: 'Cria uma Flecha em Voo com temporizador de 2 ações e 1,40x ATK (1,55x se você tinha 50 ou mais de Tensão), sem causar dano imediato.', ability: { name: 'Flecha Balística', desc: 'Cria uma Flecha em Voo de 1,40x ATK (1,55x com 50 ou mais de Tensão), sem dano imediato.', cooldown: 5, condition: { type: 'resourceAtMost', resource: 'flightCount', value: 3 }, effect: { kind: 'ballistic', dmgMult: 1.40, archerPath: 'precision', archerShotType: 'ballistic', archerFlightCount: 1, archerFlightTimer: 2, archerFlightDmgMult: 1.40, archerFlightHighTensionDmgMult: 1.55 } }, mechanicRefs: ['arqueiro:flight','arqueiro:tension'] },
      { name: 'Olho Imóvel', desc: '+4% de Dano Crítico. No Horizonte, Disparos Precisos diretos recebem +3% de dano final adicional.', effect: { critDmgPct: 0.04 }, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Disparo Pleno', desc: 'Requer e consome 70 de Tensão. Causa 2,10x ATK com +18 pontos percentuais de Crítico e 12% de penetração de DEF.', ability: { name: 'Disparo Pleno', desc: '2,10x ATK, +18 pontos percentuais de Crítico e 12% de penetração de DEF.', cooldown: 6, condition: { type: 'resourceAtLeast', resource: 'tension', value: 70 }, effect: { kind: 'bigHit', dmgMult: 2.10, archerPath: 'precision', archerShotType: 'precise', archerRequiresTension: 70, archerTensionCost: 70, archerCritBonus: 0.18, archerDefPenPct: 0.12 } }, mechanicRefs: ['arqueiro:tension'] },
      { name: 'Flecha Impossível', desc: 'Requer 100 de Tensão e consome tudo. Causa 2,25x ATK com crítico garantido e 18% de penetração de DEF, além de acelerar suas Flechas em Voo já existentes.', ability: { name: 'Flecha Impossível', desc: '2,25x ATK com crítico garantido, 18% de penetração de DEF, e acelera Flechas em Voo já existentes.', cooldown: 9, condition: { type: 'resourceAtLeast', resource: 'tension', value: 100 }, effect: { kind: 'guaranteedCrit', dmgMult: 2.25, archerPath: 'precision', archerShotType: 'precise', archerRequiresTension: 100, archerTensionCost: 100, archerDefPenPct: 0.18, archerImmediateTimerReduction: 1 } }, mechanicRefs: ['arqueiro:tension','arqueiro:flight'] },
      { name: 'Memória Muscular', desc: 'Depois de uma ofensiva que gaste 60 ou mais de Tensão e acerte, recupera 15 de Tensão.', effect: {}, mechanicRefs: ['arqueiro:tension'] },
    ]),
    buildPath('arqueiro', 'tiro-rapido', 'Tempestade', '#4f9d4f', [
      { name: 'Dedos Ligeiros', desc: '+1,5% de Velocidade de ação. Com 3 ou mais de Cadência, AGI concede até +2% adicionais.', effect: {}, mechanicRefs: ['arqueiro:cadence'] },
      { name: 'Mira Rítmica', desc: '+1,5 ponto percentual de Precisão. Em distância 1 ou 2, DES concede até +2 pontos percentuais adicionais.', effect: { accuracyPct: 0.015 }, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Pulso de Salva', desc: '+1 ponto percentual de Crítico. Com 4 ou mais de Cadência, SOR concede até +2 pontos percentuais adicionais.', effect: { critPct: 0.01 }, mechanicRefs: ['arqueiro:cadence'] },
      { name: 'Ritmo da Corda', desc: '-3% de Recarga somente para habilidades de Tempestade.', effect: {}, mechanicRefs: ['arqueiro:cadence'] },
      { name: 'Tiro Duplo', desc: 'Rajada de 2 impactos independentes de 0,68x ATK cada. Cada impacto que acertar gera +1 de Cadência (até +2). Com Ritmo Perfeito, adiciona +0,34x ao dano total.', ability: { name: 'Tiro Duplo', desc: 'Dois impactos independentes de 0,68x ATK; cada acerto gera Cadência. Com Ritmo Perfeito, +0,34x de dano total.', cooldown: 3, condition: { type: 'always' }, effect: { kind: 'multiHit', hitCount: 2, hitDmgMults: [0.68,0.68], archerPath: 'rapid', archerShotType: 'volley', archerPerfectExtraRatio: 0.34 } }, mechanicRefs: ['arqueiro:cadence'] },
      { name: 'Passo de Rajada', desc: '+1,5 ponto percentual de Evasão. Se todos os impactos de uma Rajada acertarem, +3 pontos percentuais adicionais até a próxima ação inimiga.', effect: { evasionPct: 0.015 }, mechanicRefs: ['arqueiro:cadence'] },
      { name: 'Embalo Perfeito', desc: 'Com Cadência 3, 4, 5 ou 6, concede respectivamente +2%, +4%, +6% ou +8% de Velocidade, e prepara Ritmo Perfeito ao chegar em 6.', effect: {}, mechanicRefs: ['arqueiro:cadence','arqueiro:perfect_rhythm'] },
      { name: 'Corda Solta', desc: '+2% de dano físico. Rajadas usadas em distância 1 ou 2 recebem +2% de dano final adicional.', effect: { dmgPct: 0.02 }, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Ritmo Limpo', desc: 'Quando todos os impactos originais de uma Rajada acertarem, prepara +4 pontos percentuais de Precisão para a próxima Rajada.', effect: {}, mechanicRefs: ['arqueiro:cadence'] },
      { name: 'Rajada Crescente', desc: 'Rajada de 3 impactos independentes de 0,50x, 0,55x e 0,60x ATK. Com Ritmo Perfeito, adiciona +0,30x ao dano total.', ability: { name: 'Rajada Crescente', desc: 'Três impactos independentes de 0,50x, 0,55x e 0,60x ATK. Com Ritmo Perfeito, +0,30x de dano total.', cooldown: 4, condition: { type: 'always' }, effect: { kind: 'multiHit', hitCount: 3, hitDmgMults: [0.50,0.55,0.60], archerPath: 'rapid', archerShotType: 'volley', archerPerfectExtraRatio: 0.30 } }, mechanicRefs: ['arqueiro:cadence'] },
      { name: 'Chuva de Flechas', desc: 'Com espaço para 3 Flechas em Voo, cria três Flechas com temporizadores de 1, 2 e 3 ações e 0,58x ATK cada. Não gera nem consome Cadência.', ability: { name: 'Chuva de Flechas', desc: 'Cria três Flechas em Voo de 0,58x ATK, com temporizadores de 1, 2 e 3 ações.', cooldown: 6, condition: { type: 'resourceAtMost', resource: 'flightCount', value: 1 }, effect: { kind: 'ballistic', archerPath: 'rapid', archerShotType: 'ballistic', archerFlightCount: 3, archerFlightTimer: 1, archerFlightDmgMult: 0.58, archerFlightHitDmgMults: [0.58,0.58,0.58] } }, mechanicRefs: ['arqueiro:flight'] },
      { name: 'Ombro Incansável', desc: '+8 de Vida Máxima. Com 4 ou mais de Cadência, -3% de dano direto recebido, com VIT concedendo até +2 pontos percentuais adicionais.', effect: { maxHpFlat: 8 }, mechanicRefs: ['arqueiro:cadence'] },
      { name: 'Salva Implacável', desc: 'Requer 4 ou mais de Cadência, consumindo 4. Cinco impactos de 0,42x ATK (o quinto vira 0,55x em distância 1 ou 2); com Ritmo Perfeito, adiciona metade do valor do último impacto.', ability: { name: 'Salva Implacável', desc: 'Cinco impactos de 0,42x ATK (o quinto mais forte em distância 1 ou 2); pode criar uma Flecha em Voo. Com Ritmo Perfeito, adiciona metade do último impacto.', cooldown: 7, condition: { type: 'resourceAtLeast', resource: 'cadence', value: 4 }, effect: { kind: 'multiHit', hitCount: 5, hitDmgMults: [0.42,0.42,0.42,0.42,0.42], archerPath: 'rapid', archerShotType: 'volley', archerCadenceCost: 4, archerPerfectExtraRatio: 0.5, archerFifthDistanceMult: 0.55, archerCreatesFlightOnHits: 4, archerFlightTimer: 1, archerFlightDmgMult: 0.42 } }, mechanicRefs: ['arqueiro:cadence','arqueiro:flight'] },
      { name: 'Tempestade de Penas', desc: 'Requer 4 de Cadência ou Ritmo Perfeito. Quatro impactos de 0,44x ATK; com Ritmo Perfeito, adiciona +0,22x ao dano total e acelera suas Flechas em Voo já existentes.', ability: { name: 'Tempestade de Penas', desc: 'Quatro impactos de 0,44x ATK; com Ritmo Perfeito, +0,22x de dano total e acelera Flechas em Voo já existentes.', cooldown: 9, condition: { type: 'any', conditions: [{ type: 'resourceAtLeast', resource: 'cadence', value: 4 }, { type: 'stateActive', state: 'perfectRhythm' }] }, effect: { kind: 'multiHit', hitCount: 4, hitDmgMults: [0.44,0.44,0.44,0.44], archerPath: 'rapid', archerShotType: 'volley', archerPerfectExtraRatio: 0.22, archerImmediateTimerReduction: 1 } }, mechanicRefs: ['arqueiro:cadence','arqueiro:flight'] },
      { name: 'Corda Incansável', desc: 'A convergência de 2 Flechas em Voo gera +1 de Cadência; a de 3 ou 4 Flechas gera +2, no máximo duas vezes por janela.', effect: {}, mechanicRefs: ['arqueiro:convergence','arqueiro:cadence'] },
    ]),
    buildPath('arqueiro', 'instinto', 'Vento Cortante', '#4fb8b0', [
      { name: 'Passo Leve', desc: '+1,5 ponto percentual de Evasão. Depois de mudar de distância, AGI concede até +2% adicionais até a próxima ação inimiga.', effect: { evasionPct: 0.015 }, mechanicRefs: ['arqueiro:distance','arqueiro:steps'] },
      { name: 'Olhar Lateral', desc: '+1,5 ponto percentual de Precisão. Em distância 0, reduz a penalidade de Precisão de -6 para -3 pontos percentuais.', effect: { accuracyPct: 0.015 }, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Reflexo de Caça', desc: '+1,5% de Velocidade de ação. Depois de um erro inimigo, +2% adicionais até sua próxima ação.', effect: {}, mechanicRefs: ['arqueiro:reflex'] },
      ...
      { name: 'Tiro de Recuo', desc: 'Causa 1,20x ATK (1,45x em distância 0) e depois avança sua distância em 1.', ability: { name: 'Tiro de Recuo', desc: '1,20x ATK (1,45x em distância 0); depois avança a distância em 1.', cooldown: 3, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.20, archerPath: 'instinct', archerShotType: 'precise', archerDistanceShift: 1, archerDistanceZeroMult: 1.45 } }, mechanicRefs: ['arqueiro:distance','arqueiro:steps'] },
      { name: 'Fôlego Nômade', desc: '+8 de Vida Máxima. Depois de mudar de distância, -3% de dano direto recebido até a próxima ação inimiga.', effect: { maxHpFlat: 8 }, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Passos', desc: 'Desbloqueia até 3 cargas de Passos; com as 3, +4 pontos percentuais de Evasão.', effect: {}, mechanicRefs: ['arqueiro:steps'] },
      { name: 'Tiro de Quadril', desc: '+1 ponto percentual de Crítico. Em distância 0 ou 1, o dano direto recebe até +2% adicionais por DES.', effect: { critPct: 0.01 }, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Reflexo Preparado', desc: 'Desbloqueia Reflexo: um erro direto do inimigo prepara +8 pontos percentuais de Precisão para a próxima ofensiva, por até 2 ações.', effect: {}, mechanicRefs: ['arqueiro:reflex'] },
      { name: 'Passo Fantasma', desc: 'Manobra usável em distância 0 ou com sua vida abaixo de 50%. Avança sua distância em 2, consome até 2 Passos e penaliza a próxima ação inimiga.', ability: { name: 'Passo Fantasma', desc: 'Reposiciona-se e consome Passos disponíveis; penaliza a próxima ação inimiga.', cooldown: 8, condition: { type: 'any', conditions: [{ type: 'resourceAtMost', resource: 'distance', value: 0 }, { type: 'hpBelow', pct: 0.5 }] }, effect: { kind: 'archerMove', archerPath: 'instinct', archerShotType: 'maneuver', archerDistanceShift: 2, archerConsumesSteps: 2 } }, mechanicRefs: ['arqueiro:distance','arqueiro:steps'] },
      { name: 'Tiro à Queima-Roupa', desc: 'Requer distância 1 ou menos. Causa 1,65x ATK em distância 1, ou 2,10x em distância 0; depois avança sua distância em 2.', ability: { name: 'Tiro à Queima-Roupa', desc: '1,65x ATK em distância 1, ou 2,10x em distância 0; depois avança a distância em 2.', cooldown: 5, condition: { type: 'resourceAtMost', resource: 'distance', value: 1 }, effect: { kind: 'bigHit', dmgMult: 1.65, archerPath: 'instinct', archerShotType: 'precise', archerDistanceShift: 2, archerDistanceZeroMult: 2.10 } }, mechanicRefs: ['arqueiro:distance'] },
      { name: 'Ancoragem em Movimento', desc: '+2% de DEF. O custo de uma mudança voluntária de distância cai de 15 para 8. Com 3 Passos, +2% de Velocidade adicional.', effect: { defPct: 0.02 }, mechanicRefs: ['arqueiro:distance','arqueiro:steps'] },
      { name: 'Flecha de Vento', desc: 'Causa 1,45x ATK, avança sua distância em 1 e aproxima os temporizadores das suas Flechas em Voo já existentes.', ability: { name: 'Flecha de Vento', desc: '1,45x ATK; avança a distância em 1 e aproxima os temporizadores de Flechas em Voo existentes.', cooldown: 5, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.45, archerPath: 'instinct', archerShotType: 'precise', archerDistanceShift: 1, archerAlignFlights: true } }, mechanicRefs: ['arqueiro:distance','arqueiro:flight'] },
      { name: 'Contra-Flecha', desc: 'Requer Reflexo pronto e 1 Passo disponível; consome ambos. Causa 1,90x ATK com +10 pontos percentuais de Precisão, avança sua distância em 1 e acelera a Flecha em Voo mais antiga.', ability: { name: 'Contra-Flecha', desc: '1,90x ATK e +10 pontos percentuais de Precisão; avança a distância em 1 e acelera a Flecha em Voo mais antiga.', cooldown: 6, condition: { type: 'all', conditions: [{ type: 'stateActive', state: 'reflex' }, { type: 'resourceAtLeast', resource: 'steps', value: 1 }] }, effect: { kind: 'bigHit', dmgMult: 1.90, archerPath: 'instinct', archerShotType: 'precise', archerDistanceShift: 1, archerConsumesSteps: 1, archerConsumesReflex: true, archerCritBonus: 0.10, archerAccelerateOldest: true } }, mechanicRefs: ['arqueiro:reflex','arqueiro:steps'] },
      { name: 'Dono do Espaço', desc: 'Uma vez por inimigo, impede que ele feche a distância de 1 para 0, concede 3 Passos, prepara Reflexo e acelera a Flecha em Voo mais antiga.', effect: {}, mechanicRefs: ['arqueiro:distance','arqueiro:steps','arqueiro:reflex'] },
    ]),
  ],
  cacador: [
    ...
    buildPath('cacador', 'rastreio', 'Rastreio', '#5b5f6a', [
      ...
      { name: 'Sumir na Mata', desc: 'Habilidade ativa: +15 pontos percentuais de evasão por 2 ciclos. Concede imediatamente +2 Rastro (respeitando o máximo de 5). Recarga de 6 ciclos.',
        mechanicRefs: ['cacador:trail'],
        ability: {
          name: 'Sumir na Mata', desc: '+15 pontos percentuais de evasão por 2 ciclos e +2 Rastro imediato.',
          cooldown: 6, condition: { type: 'always' },
      ...
      { name: 'Passo Etéreo', desc: 'Habilidade ativa: só pode ser usada com sua vida abaixo de 45% — +25 pontos percentuais de evasão por 2 ciclos. Concede +1 Rastro imediato; o primeiro ataque direto inimigo que errar durante o efeito concede +1 Rastro adicional (e +1 Brecha, se desbloqueada). Recarga de 10 ciclos.',
        mechanicRefs: ['cacador:trail', 'cacador:breaches'],
        ability: {
          name: 'Passo Etéreo', desc: '+25 pontos percentuais de evasão por 2 ciclos quando sua vida está abaixo de 45%. Gera Rastro (e possivelmente Brecha) no primeiro erro inimigo.',
      ...
      { name: 'Manto das Sombras', desc: 'Habilidade ativa: +20 pontos percentuais de evasão por 3 ciclos. Durante o efeito, cada ação inimiga que errar um ataque direto concede +1 Rastro (e +1 Brecha, se desbloqueada, até 2 Brechas por uso, máximo uma por ação inimiga). Recarga de 10 ciclos.',
        mechanicRefs: ['cacador:trail', 'cacador:breaches'],
        ability: {
          name: 'Manto das Sombras', desc: '+20 pontos percentuais de evasão por 3 ciclos. Cada erro inimigo durante o efeito gera Rastro (e possivelmente Brecha).',
      ...
      { name: 'Um com a Caça', desc: 'Habilidade ativa: só pode ser usada com Rastro = 5 — por 4 ciclos, contra a presa atual: +8% de dano direto, +8% de velocidade de ação, +12 pontos percentuais de evasão. Não consome Rastro, não afeta DOT, não carrega para outro inimigo. Recarga de 12 ciclos.',
        mechanicRefs: ['cacador:trail'],
        ability: {
          name: 'Um com a Caça', desc: 'Por 4 ciclos: +8% dano, +8% velocidade, +12 pontos percentuais de evasão contra a presa atual.',
  ...
  // Legacy pre-redesign tree retained only in source history; the live
  // table is assigned declaratively below (SKILL_TREES.feiticeiro = [...]),
  // same convention as necromante/bardo/druida.
  feiticeiro: [],
  // Legacy pre-redesign tree retained only in source history; the live
  // table is assigned declaratively below (SKILL_TREES.bruxo = [...]),
  // same convention as necromante/bardo/druida.
  bruxo: [],
  // Legacy pre-redesign tree retained only in source history; the live
  // table is assigned declaratively below (SKILL_TREES.druida = [...]),
  // same convention as necromante/bardo.
  druida: [],
...
const necroInt = (detail: string, label = 'Mecânica'): ScalingEntry[] => [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o MATK usado por esta técnica.' }, { label, role: 'mecanica', description: detail }];
...
SKILL_TREES.necromante = [
  buildPath('necromante', 'decomposicao', 'Decomposição', '#3a3a4a', [
    { name: 'Ímpeto Necrótico', desc: '+2% de MATK. Habilidades diretas que aplicam Decomposição recebem até +3% de dano final conforme INT.', effect: { magicDmgPct: 0.02 }, mechanicRefs: ['necromante:decomposition'], scaling: necroInt('+0,075 ponto percentual por INT, máximo +3%.', 'Decomposição') },
    ...
    { name: 'Corrosão Profunda', desc: '+2,5% de MATK. Contra 3+ Decomposições, dano mágico direto recebe até +3% conforme INT.', effect: { magicDmgPct: 0.025 }, mechanicRefs: ['necromante:decomposition'], scaling: necroInt('+0,075 ponto percentual por INT, máximo +3%; não aumenta a Praga.', 'Decomposição') },
    { name: 'Praga Necrótica', desc: 'Cause 1,10x MATK, aplique 2 Decomposições e uma Praga de 0,16x MATK por 4 ciclos.', ability: { name: 'Praga Necrótica', desc: '1,10x MATK, +2 Decomposições e Praga Necrótica.', cooldown: 4, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.10, dmgType: 'magical', decompositionOnHit: 2, plagueApply: true, plagueMultiplier: 0.16, plagueDuration: 4, necromancerTag: 'decomposition' } }, mechanicRefs: ['necromante:decomposition', 'necromante:plague'], scaling: necroInt('A Praga guarda o MATK atual e Decomposição fortalece cada tick.', 'Praga') },
    ...
    { name: 'Dreno Necrótico', desc: 'Cada tick da Praga cura 8% do dano real, limitado a 0,75% da Vida Máxima.', mechanicRefs: ['necromante:plague'], scaling: necroInt('A cura cresce somente através do dano real da Praga.', 'Praga') },
    { name: 'Essência Necrótica', desc: '+2,5% de MATK. Praga passa a 0,17x; Praga Devastadora, 0,21x.', effect: { magicDmgPct: 0.025 }, mechanicRefs: ['necromante:plague'], scaling: necroInt('Altera o coeficiente, não a duração.', 'Praga') },
    { name: 'Amplificação da Praga', desc: 'Tick da Praga com 4+ Decomposições aplica -6% ATK inimigo até o próximo ciclo.', mechanicRefs: ['necromante:decomposition', 'necromante:plague'], scaling: necroInt('O debuff não acumula e só é renovado por novo tick válido.', 'Praga') },
    { name: 'Toque Corrosivo', desc: 'Cause 1,55x MATK, aplique 1 Decomposição e reduza MDEF em 2% por stack, até 10%, por 3 ciclos.', ability: { name: 'Toque Corrosivo', desc: '1,55x MATK, +1 Decomposição e corrosão de MDEF.', cooldown: 4, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.55, dmgType: 'magical', decompositionOnHit: 1, necromancerTag: 'decomposition' } }, mechanicRefs: ['necromante:decomposition'], scaling: necroInt('A redução de MDEF lê os stacks de Decomposição já aplicados no alvo.', 'Decomposição') },
    { name: 'Praga Devastadora', desc: 'Custa 1 Alma. Requer Praga e 2 Decomposições. 1,25x MATK, +2 stacks e Praga de 0,20x por 5 ciclos.', ability: { name: 'Praga Devastadora', desc: 'Investe uma Alma para intensificar a Praga.', cooldown: 6, condition: { type: 'all', conditions: [{ type: 'resourceAtLeast', resource: 'souls', value: 1 }, { type: 'periodicEffectActive', effectId: 'necromante:plague' }, { type: 'enemyStacksAtLeast', stackId: 'decomposition', stacks: 2 }] }, effect: { kind: 'bigHit', dmgMult: 1.25, dmgType: 'magical', soulCost: 1, decompositionOnHit: 2, plagueApply: true, plagueMultiplier: 0.20, plagueDuration: 5, necromancerTag: 'decomposition' } }, mechanicRefs: ['necromante:souls', 'necromante:decomposition', 'necromante:plague'], scaling: necroInt('Com Essência Necrótica, a nova Praga fica ainda mais forte (0,21x).', 'Praga') },
    { name: 'Poder da Ruína', desc: '+3% de MATK. Técnica de Decomposição que gasta Alma contra 5 stacks recebe até +3% dano direto conforme INT.', effect: { magicDmgPct: 0.03 }, mechanicRefs: ['necromante:souls', 'necromante:decomposition'], scaling: necroInt('+0,075 ponto percentual por INT, máximo +3%.', 'Decomposição') },
    { name: 'Colheita das Almas', desc: 'Requer 2 Decomposições. 1,60x MATK e +0,16x por até 3 stacks consumidos; consumir 3 gera 1 Alma.', ability: { name: 'Colheita das Almas', desc: 'Consome até 3 Decomposições para dano e uma possível Alma.', cooldown: 5, condition: { type: 'enemyStacksAtLeast', stackId: 'decomposition', stacks: 2 }, effect: { kind: 'bigHit', dmgMult: 1.60, dmgType: 'magical', decompositionConsumeMax: 3, soulGainOnConsumeExact: 3, necromancerTag: 'decomposition' } }, mechanicRefs: ['necromante:decomposition', 'necromante:souls'], scaling: necroInt('Cada stack consumido soma +0,16x ao dano, até um total de 2,08x.', 'Decomposição') },
    { name: 'Apocalipse Necrótico', desc: 'Custa 3 Almas. Requer 5 stacks e Praga. 1,85x MATK +70% do dano restante da Praga, limitado a 1,00x MATK; consome setup ao acertar.', ability: { name: 'Apocalipse Necrótico', desc: 'Detona a Praga e toda a Decomposição.', cooldown: 9, condition: { type: 'all', conditions: [{ type: 'resourceAtLeast', resource: 'souls', value: 3 }, { type: 'enemyStacksEqual', stackId: 'decomposition', stacks: 5 }, { type: 'periodicEffectActive', effectId: 'necromante:plague' }] }, effect: { kind: 'bigHit', dmgMult: 1.85, dmgType: 'magical', soulCost: 3, plagueDetonatePct: 0.70, plagueDetonateCapMult: 1, necromancerTag: 'decomposition' } }, mechanicRefs: ['necromante:souls', 'necromante:decomposition', 'necromante:plague'], scaling: necroInt('O bônus de dano vem dos ticks restantes da Praga, com um teto de 1,00x MATK.', 'Praga') },
  ...
  buildPath('necromante', 'drenar-vida', 'Drenar Vida', '#4a2a5a', [
    ...
    { name: 'Vínculo Funerário', desc: '+2% de MDEF. Dano dos Servos recebe até +4% conforme INT.', effect: { mdefPct: 0.02 }, mechanicRefs: ['necromante:servants'], scaling: necroInt('+0,10 ponto percentual por INT, máximo +4%.', 'Servos') },
    ...
    { name: 'Retribuição dos Mortos', desc: 'Ataques de Servo acumulam +3% para a próxima magia direta, até +9%; a magia consome tudo.', mechanicRefs: ['necromante:servants'], scaling: necroInt('O bônus só vem de ataques de Servo; Praga e seu próprio ataque físico não contam.', 'Servos') },
    { name: 'Sangue Pelo Sangue', desc: 'Sacrifica o Servo mais antigo para 1,85x MATK +0,08x por ataque restante e cura 22%; sem Servo custa 1 Alma, causa 1,60x e cura 15%.', ability: { name: 'Sangue Pelo Sangue', desc: 'Sacrifício ofensivo que drena vida.', cooldown: 5, condition: { type: 'any', conditions: [{ type: 'summonCountAtLeast', count: 1 }, { type: 'resourceAtLeast', resource: 'souls', value: 1 }] }, effect: { kind: 'bigHit', dmgMult: 1.60, dmgType: 'magical', soulCost: 1, sacrificeOldestSummon: true, directHealFromDamagePct: 0.15, directHealCapPct: 0.06, necromancerTag: 'soul' } }, mechanicRefs: ['necromante:souls', 'necromante:servants'], scaling: necroInt('Sacrificar um Servo com ataques restantes eleva o golpe até 2,17x e a cura até 22% (teto de 8% da Vida Base).', 'Servos') },
    { name: 'Véu da Morte', desc: 'Custa 1 Alma. Por 3 ciclos, reduz dano direto em 8%; um Servo pode perder um ataque para elevar um golpe a 15%.', ability: { name: 'Véu da Morte', desc: 'Sacrifica vida útil dos Servos por proteção.', cooldown: 8, condition: { type: 'resourceAtLeast', resource: 'souls', value: 1 }, effect: { kind: 'deathVeil', soulCost: 1, buffRounds: 3 } }, mechanicRefs: ['necromante:souls', 'necromante:servants'], scaling: necroWis('Proteção fixa, uma ativação por ação inimiga.') },
  ...
  buildPath('necromante', 'ceifador', 'Ceifador', '#c89a2e', [
    ...
    { name: 'Poder da Ceifa', desc: '+2% de MATK. Técnicas ofensivas que gastam Alma recebem até +3% conforme INT.', effect: { magicDmgPct: 0.02 }, mechanicRefs: ['necromante:souls'], scaling: necroInt('+0,075 ponto percentual por INT, máximo +3%.', 'Almas') },
    { name: 'Manto das Almas', desc: '+2% de MDEF e +0,5% por Alma armazenada, até +3% adicional.', effect: { mdefPct: 0.02 }, mechanicRefs: ['necromante:souls'], scaling: necroWis('A defesa extra sobe com o número de Almas guardadas no momento.') },
    { name: 'Golpe Calculado', desc: '+3% de Dano Crítico e +0,5% a cada 20% de Vida perdida da presa, até +2% adicional.', effect: { critDmgPct: 0.03 }, scaling: necroLuck('O bônus extra cresce conforme a presa perde Vida.') },
    { name: 'Golpe da Foice', desc: 'Cause 1,45x MATK. O primeiro crítico desta técnica em cada inimigo gera 1 Alma.', ability: { name: 'Golpe da Foice', desc: 'Golpe de ceifa que pode arrancar uma Alma no crítico.', cooldown: 4, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.45, dmgType: 'magical', necromancerTag: 'reaper' } }, mechanicRefs: ['necromante:souls'], scaling: necroInt('SOR aumenta a chance de crítico, que é o que gera a Alma.', 'Almas') },
    { name: 'Essência Ceifada', desc: '+2,5% de MATK. Ganhar Alma prepara até +3% de dano na próxima magia direta por 2 ciclos.', effect: { magicDmgPct: 0.025 }, mechanicRefs: ['necromante:souls'], scaling: necroInt('+0,075 ponto percentual por INT, máximo +3%.', 'Almas') },
    { name: 'Ceifa Ampliada', desc: 'Quando um ataque cruza limite de Alma, próxima magia ganha +8% Dano Crítico por Alma gerada, até +16%, por 2 ciclos.', mechanicRefs: ['necromante:souls'], scaling: necroLuck('O bônus é preparado assim que você cruza um marco de geração de Alma.') },
...
SKILL_TREES.ladino = [
  buildPath('ladino', 'veneno', 'Assassino', '#4f7a3a', [
    ...
    { name: 'Passo Sombrio', desc: 'AÇÃO RÁPIDA. Entre em Furtivo. Só é usada se você ainda não estiver Furtivo.', ability: { name: 'Passo Sombrio', desc: 'Entre em Furtivo sem substituir sua próxima Ação Principal.', cooldown: 5, actionType: 'quick', condition: { type: 'stateInactive', state: 'stealth' }, effect: { kind: 'rogueStealth', roguePath: 'assassin' } }, mechanicRefs: ['ladino:initiative', 'ladino:stealth'] },
    ...
    { name: 'Lâmina Envenenada', desc: 'AÇÃO RÁPIDA. Prepare Lâmina Tóxica para a próxima Principal física que acertar. A Toxina dura 3 ciclos.', ability: { name: 'Lâmina Envenenada', desc: 'Prepare uma Toxina de 0,12x ATK; em Emboscada, 0,15x.', cooldown: 4, actionType: 'quick', condition: { type: 'stateInactive', state: 'toxicBlade' }, effect: { kind: 'rogueToxicBlade', roguePath: 'assassin', toxicBlade: true } }, mechanicRefs: ['ladino:initiative', 'ladino:toxin'] },
  ...
  buildPath('ladino', 'laminas', 'Trapaceiro', '#c89a2e', [
    ...
    { name: 'Mão Leve', desc: 'AÇÃO PRINCIPAL. 1,30x ATK; com Vantagem, 1,60x e, ao acertar, atrasa a próxima ação inimiga em 18% e reduz uma Rápida.', ability: { name: 'Mão Leve', desc: 'Rouba Tempo uma vez da ação inimiga já agendada.', cooldown: 5, actionType: 'main', condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.30, advantageDmgMult: 1.60, timeSteal: true, offensive: true, roguePath: 'trickster', imageEchoRatio: 0.10 } }, mechanicRefs: ['ladino:initiative', 'ladino:advantage', 'ladino:time_stolen'] },
    { name: 'Ás na Manga', desc: 'AÇÃO PRINCIPAL. Requer Vantagem ou Vida inimiga até 25%. Causa 2,20x, 2,55x ou 2,75x; com Vantagem, +10 pontos percentuais de Crítico e 12% de penetração.', ability: { name: 'Ás na Manga', desc: 'Finalizador do Trapaceiro; não garante acerto nem crítico.', cooldown: 8, actionType: 'main', condition: { type: 'any', conditions: [{ type: 'advantageReady' }, { type: 'enemyHpBelow', pct: 0.25 }] }, effect: { kind: 'bigHit', dmgMult: 2.20, advantageDmgMult: 2.55, combinedDmgMult: 2.75, advantageCritPct: 0.10, advantageDefPenPct: 0.12, offensive: true, roguePath: 'trickster', imageEchoRatio: 0.10 } }, mechanicRefs: ['ladino:initiative', 'ladino:advantage'] },
    { name: 'Mestre do Improviso', desc: 'Principal Trapaceira com Vantagem recebe +0,10x ATK e +8 pontos percentuais de Precisão. Se a habilidade inteira errar, Vantagem é restaurada depois.', mechanicRefs: ['ladino:advantage'] },
  ]),
];
...
SKILL_TREES.paladino = [
  ...
  buildPath('paladino', 'martelo', 'Veredito', '#a5432f', [
    ...
    { name: 'Veredito', desc: 'VEREDITO. Requer Convicção 1. Consome as Virtudes e causa 2,25x ATK com 2 ou 2,70x com 3. Pleno com Justiça Regente: 2,95x e +12pp de penetração. Recarga: 6 ciclos.', ability: { name: 'Veredito', desc: 'Consome a Liturgia no início. Causa 2,25x ATK com 2 Virtudes ou 2,70x com 3; com uma Virtude, usa a forma inicial.', cooldown: 6, condition: { type: 'resourceAtLeast', resource: 'conviction', value: 1 }, effect: { kind: 'bigHit', paladinPath: 'verdict', paladinVerdict: true, paladinRadiant: true, verdictDmgMultByConviction: { 1: 1.90, 2: 2.25, 3: 2.70 }, fullJusticeDmgMult: 2.95 } }, mechanicRefs: ['paladino:verdict', 'paladino:conviction', 'paladino:regent'], scaling: paladinStrikeScaling('Convicção e Regente são capturadas no início, mesmo se errar.') },
  ...
  buildPath('paladino', 'luz', 'Redenção', '#d8c27a', [
    ...
    { name: 'Fôlego Misericordioso', desc: '+8 de Vida Máxima. VIT aumenta curas deste caminho baseadas na Vida Máxima em 0,08 ponto percentual por ponto, até +3 pontos percentuais.', effect: { maxHpFlat: 8 }, mechanicRefs: ['paladino:virtues'], scaling: paladinHealScaling('O bônus é limitado e não transforma o Paladino em curador principal.') },
    ...
    { name: 'Luz que Não Cede', desc: 'Invoca Misericórdia e causa 1,30x ATK. Ao acertar, cura 8% da Vida Máxima. Abaixo de 70% de Vida no início, também invoca Coragem, abrindo naturalmente o Veredito da Redenção. Recarga: 7 ciclos.', ability: { name: 'Luz que Não Cede', desc: 'Invoca Misericórdia e causa 1,30x ATK; ao acertar, cura 8% da Vida Máxima. Abaixo de 70%, também invoca Coragem.', cooldown: 7, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.30, paladinPath: 'redemption', paladinVirtues: ['mercy'], paladinExtraVirtueBelowHp: { virtue: 'courage', pct: 0.70 }, activeHealMaxHpPct: 0.08 } }, mechanicRefs: ['paladino:virtues', 'paladino:liturgy', 'paladino:regent'], scaling: paladinStrikeScaling('Coragem é adicionada depois de Misericórdia e se torna a Regente quando a Vida cai abaixo de 70%.') },
...
export function computeSkillBonuses(classId: ClassId, unlocked: string[]): Required<Omit<SkillEffect, 'dmgPctVsStatus'>> & { dmgPctVsPoison: number; dmgPctVsBurn: number } {
  const totals = {
    ...
    evasionPct: 0, accuracyPct: 0, cooldownReductionPct: 0, healingPowerPct: 0,
  ...
  for (const path of SKILL_TREES[classId]) {
    for (const n of path.nodes) {
      ...
      totals.healingPowerPct += n.effect.healingPowerPct ?? 0;
...
// Druida — O Ciclo Vivo. Redesign completo: Estações -> Sintonia -> Ano
// Perfeito -> Renovo, com três especializações (Jardim Vivo, Metamorfose,
// Descompasso/Reequilíbrio) que se apoiam no MESMO ciclo — ver lib/druid.ts
// para toda a matemática e combatEngine.ts para a pipeline de resolução.
// IDs 0..14 e a topologia universal (TOPOLOGY acima) são preservados
// exatamente; só o conteúdo de cada nó muda.
SKILL_TREES.druida = [
  buildPath('druida', 'cura-natural', 'Renascimento', '#3f8a5a', [
    { name: 'Raiz Profunda', desc: '+18 Vida Máxima. +2% de eficiência de cura.', effect: { maxHpFlat: 18, healingPowerPct: 0.02 },
      mechanicRefs: ['druida:garden'],
      scaling: [{ attribute: 'vit', label: 'VIT', role: 'secundario', description: 'Vida máxima base do Ciclo Vivo.' }] },
    { name: 'Seiva Serena', desc: '+2% de defesa mágica. Curas ativas de Renascimento recebem eficiência adicional por SAB total (até +3 pontos percentuais).', effect: { mdefPct: 0.02 },
      scaling: [{ attribute: 'wis', label: 'SAB', role: 'principal', description: 'Aumenta a eficiência final das curas ativas de Renascimento (até +3pp).' }] },
    { name: 'Casca Fresca', desc: '+2% de defesa.', effect: { defPct: 0.02 },
      mechanicRefs: ['druida:garden'] },
    { name: 'Ritmo da Seiva', desc: '-3% de recarga somente para habilidades de Renascimento.', effect: { cooldownReductionPct: 0.03 } },
    { name: 'Semeadura Vital', desc: 'Habilidade ativa: cura direta de 18% da Vida Base. Depois, planta 1 Semente no Jardim (2 se Sintonizada). Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:garden'],
      ability: { name: 'Semeadura Vital', desc: 'Cura 18% da Vida Base; depois planta 1 Semente (2 se Sintonizada). Primavera.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'heal', healPct: 0.18, druidSeason: 'spring', druidPath: 'rebirth', druidPlantSeeds: 1, druidPlantSeedsAligned: 2 } },
      scaling: [{ attribute: 'wis', label: 'SAB', role: 'principal', description: 'Aumenta a cura direta.' }] },
    { name: 'Carvalho Vivo', desc: '+2% de defesa. Enquanto houver ao menos 1 Fruto no Jardim, recebe 3% a menos de dano direto.', effect: { defPct: 0.02 },
      mechanicRefs: ['druida:garden'] },
    ...
    { name: 'Fôlego Perene', desc: '+18 Vida Máxima. +2% de eficiência de cura.', effect: { maxHpFlat: 18, healingPowerPct: 0.02 },
    ...
    { name: 'Seiva Ascendente', desc: 'Habilidade ativa: 1,20x de MATK (1,35x se Sintonizada). Cura 18% do dano causado (7% se Sintonizada, sempre limitada a 5%/7% da Vida Máxima). Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:garden'],
      ability: { name: 'Seiva Ascendente', desc: '1,20x MATK; cura 18% do dano causado (cap 5% da Vida Máxima). Verão: 1,35x e cap 7%.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'bigHit', dmgMult: 1.20, dmgMultAligned: 1.35, druidSeason: 'summer', druidPath: 'rebirth', healFromDamagePct: 0.18, healFromDamageCapPct: 0.05 } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano do golpe.' }] },
    { name: 'Colheita Ancestral', desc: 'Habilidade ativa: causa 1,10x de MATK (1,25x se Sintonizada). Depois, consome Frutos disponíveis um por um enquanto houver HP faltando (16%/18% da Vida Base cada). Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:garden'],
      ability: { name: 'Colheita Ancestral', desc: '1,10x MATK; depois consome Frutos até restaurar a Vida Máxima ou esgotar o Jardim. Outono: 1,25x e 18% por Fruto.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'bigHit', dmgMult: 1.10, dmgMultAligned: 1.25, druidSeason: 'autumn', druidPath: 'rebirth', druidHarvest: true, druidHarvestHealPctPerFruit: 0.16, druidHarvestHealPctPerFruitAligned: 0.18 } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano do golpe.' }, { label: 'Jardim', role: 'mecanica', description: 'Colhe Frutos disponíveis logo depois de bater.' }] },
    { name: 'Sabedoria da Colheita', desc: '+2% de defesa mágica. Curas vindas de Frutos recebem +2% de eficiência final.', effect: { mdefPct: 0.02 } },
    { name: 'Dormência Profunda', desc: 'Habilidade ativa: aplica regeneração de 4% da Vida Máxima por ciclo, por 3 ciclos. Se Sintonizada, cura imediatamente 4% da Vida Máxima antes da regeneração. Recarga de 5 ciclos.',
      mechanicRefs: ['druida:season'],
      ability: { name: 'Dormência Profunda', desc: 'Regeneração de 4% da Vida Máxima por 3 ciclos. Inverno: cura imediata de 4% antes da regeneração.',
        cooldown: 5, condition: { type: 'always' },
        effect: { kind: 'regen', regenPct: 0.04, regenRounds: 3, druidSeason: 'winter', druidPath: 'rebirth', druidImmediateHealPct: undefined, druidImmediateHealPctAligned: 0.04 } } },
    { name: 'Árvore Ancestral', desc: 'Habilidade ativa (cycle, requer Renovo): consome o Renovo, cura 22% da Vida Base, transforma todo o Jardim em Fruto e abre a Copa Ancestral por 4 ações (Frutos curam +25%; o primeiro Fruto consumido nessa janela não é removido). Recarga de 9 ciclos.',
      mechanicRefs: ['druida:renewal', 'druida:garden'],
      ability: { name: 'Árvore Ancestral', desc: 'Consome Renovo. Cura 22% da Vida Base, amadurece todo o Jardim em Fruto e abre a Copa Ancestral (4 ações, Frutos +25% de cura).',
        cooldown: 9, condition: { type: 'resourceAtLeast', resource: 'renewal', value: 1 },
        effect: { kind: 'heal', healPct: 0.22, druidSeason: 'cycle', druidPath: 'rebirth', druidTreeOfLife: true } },
      scaling: [{ label: 'Renovo', role: 'mecanica', description: 'Só pode ser usada com 1 Renovo disponível; consome-o de qualquer forma.' }] },
    { name: 'Nada se Perde', desc: 'Uma vez por Ano: se uma mesma ação consumir 2 ou mais Frutos, planta 1 nova Semente (não cresce nessa mesma ação).', effect: {},
      mechanicRefs: ['druida:garden', 'druida:attunement'] },
  ]),
  buildPath('druida', 'furia-natureza', 'Metamorfose', '#8a3a2a', [
    { name: 'Sangue Mutável', desc: '+2% de dano mágico direto das habilidades de Metamorfose. Com uma Forma ativa, INT total adiciona até +3% adicionais.', effect: { magicDmgPct: 0.02 },
      mechanicRefs: ['druida:form'],
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano mágico enquanto uma Forma estiver ativa (até +3%).' }] },
    { name: 'Pele Ancestral', desc: '+18 Vida Máxima. Enquanto qualquer Forma estiver ativa, +2% de defesa mágica. +2% de eficiência de cura.', effect: { maxHpFlat: 18, healingPowerPct: 0.02 },
      mechanicRefs: ['druida:form'] },
    { name: 'Olhos Ferais', desc: '+1,5 ponto percentual de precisão. Na Forma da Coruja, SAB total concede até +2 pontos percentuais adicionais.', effect: { accuracyPct: 0.015 },
      mechanicRefs: ['druida:form'],
      scaling: [{ attribute: 'wis', label: 'SAB', role: 'secundario', description: 'Aumenta a precisão na Forma da Coruja (até +2pp).' }] },
    { name: 'Ritmo Mutável', desc: '-3% de recarga somente para habilidades de Metamorfose.', effect: { cooldownReductionPct: 0.03 } },
    { name: 'Investida do Cervo', desc: 'Habilidade ativa: assume a Forma do Cervo. Causa 1,10x de MATK (1,20x se Sintonizada) e cura 5% da Vida Base (8% se Sintonizada). Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:form', 'druida:instinct'],
      ability: { name: 'Investida do Cervo', desc: 'Assume a Forma do Cervo. 1,10x MATK e cura 5% da Vida Base. Primavera: 1,20x e 8%.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'bigHit', dmgMult: 1.10, dmgMultAligned: 1.20, druidSeason: 'spring', druidPath: 'metamorphosis', druidFormOnCast: 'stag', healFromDamagePct: 0, healPctAligned: 0.08, healPct: 0.05 } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano do golpe.' }] },
    { name: 'Couro das Quatro Feras', desc: '+2% de defesa. Enquanto qualquer Forma estiver ativa, +2% de defesa mágica adicional.', effect: { defPct: 0.02 },
      mechanicRefs: ['druida:form'] },
    { name: 'Instinto Ancestral', desc: 'Desbloqueia o Instinto Ancestral (0 a 3). A primeira Forma assumida não gera; cada troca posterior para uma Forma diferente gera +1 (no máximo +1 por cast).', effect: {},
      mechanicRefs: ['druida:instinct', 'druida:form'] },
    { name: 'Passos do Lobo', desc: '+1,5% de velocidade de ação. Na Forma do Lobo, +1,5% adicional.', effect: {},
      mechanicRefs: ['druida:form'] },
    { name: 'Muda Completa', desc: 'Uma vez por Ano: quando uma transformação Sintonizada mudar realmente de Forma, reduz em 1 ciclo o efeito negativo removível mais grave em você (DOT > debuff de atributo > silêncio).', effect: {},
      mechanicRefs: ['druida:form', 'druida:attunement'] },
    { name: 'Presas do Lobo', desc: 'Habilidade ativa: assume a Forma do Lobo. Dois golpes de 0,62x de MATK (um terceiro golpe de 0,32x se Sintonizada). Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:form', 'druida:instinct'],
      ability: { name: 'Presas do Lobo', desc: 'Assume a Forma do Lobo. Dois golpes de 0,62x MATK. Verão: um terceiro golpe de 0,32x.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'multiHit', hitCount: 2, hitDmgMults: [0.62, 0.62], hitCountAligned: 3, hitDmgMultsAligned: [0.62, 0.62, 0.32], druidSeason: 'summer', druidPath: 'metamorphosis', druidFormOnCast: 'wolf' } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano de cada golpe.' }] },
    { name: 'Queda do Urso', desc: 'Habilidade ativa: assume a Forma do Urso. Causa 1,70x de MATK (1,90x se Sintonizada). Se Sintonizada e acertar, a redução de dano do Urso soma +2 pontos percentuais até a próxima ação inimiga. Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:form', 'druida:instinct'],
      ability: { name: 'Queda do Urso', desc: 'Assume a Forma do Urso. 1,70x MATK. Outono: 1,90x e +2 pontos percentuais de redução de dano até a próxima ação inimiga.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'bigHit', dmgMult: 1.70, dmgMultAligned: 1.90, druidSeason: 'autumn', druidPath: 'metamorphosis', druidFormOnCast: 'bear', druidBearWindowBonusPct: 0.02 } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano do golpe.' }] },
    { name: 'Espírito da Coruja', desc: '+2% de defesa mágica. Enquanto a Forma da Coruja estiver ativa, +2 pontos percentuais de precisão adicionais.', effect: { mdefPct: 0.02 },
      mechanicRefs: ['druida:form'] },
    { name: 'Olhos da Coruja', desc: 'Habilidade ativa: assume a Forma da Coruja. Causa 1,40x de MATK com 10% de penetração de MDEF (1,55x se Sintonizada, +6 pontos percentuais de precisão só nesse cast). Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:form', 'druida:instinct'],
      ability: { name: 'Olhos da Coruja', desc: 'Assume a Forma da Coruja. 1,40x MATK, 10% de penetração de MDEF. Inverno: 1,55x e +6 pontos percentuais de precisão só nesse cast.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'bigHit', dmgMult: 1.40, dmgMultAligned: 1.55, mdefPenPct: 0.10, druidAccuracyBonus: 0, druidAccuracyBonusAligned: 0.06, druidSeason: 'winter', druidPath: 'metamorphosis', druidFormOnCast: 'owl' } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano do golpe.' }] },
    { name: 'Avatar Primordial', desc: 'Habilidade ativa (cycle, requer Instinto 3): consome os 3 Instintos e causa 2,00x de MATK (2,25x se Renovo disponível, consumindo-o). Depois, assume as quatro Formas simultaneamente por 3 ações (4 se Renovo consumido). Recarga de 9 ciclos.',
      mechanicRefs: ['druida:instinct', 'druida:avatar', 'druida:renewal', 'druida:form'],
      ability: { name: 'Avatar Primordial', desc: 'Consome os 3 Instintos. 2,00x MATK; depois, Cervo+Lobo+Urso+Coruja simultâneos por 3 ações. Com Renovo: 2,25x e 4 ações, consumindo o Renovo.',
        cooldown: 9, condition: { type: 'resourceAtLeast', resource: 'instinct', value: 3 },
        effect: { kind: 'bigHit', dmgMult: 2.00, dmgMultRenewed: 2.25, druidSeason: 'cycle', druidPath: 'metamorphosis', druidFormOnCastAll: true, druidAvatar: true } },
      scaling: [{ label: 'Instinto', role: 'mecanica', description: 'Requer e consome os 3 Instintos; a versão com Renovo é ainda mais forte.' }] },
    { name: 'Não Existe Forma Final', desc: 'Quando o Avatar Primordial termina, o Instinto Ancestral volta para 1 (em vez de 0) e você assume automaticamente a Forma da Estação atual — isso não é ação, não gera Instinto nem Sintonia.', effect: {},
      mechanicRefs: ['druida:avatar', 'druida:instinct', 'druida:form'] },
  ]),
  buildPath('druida', 'equilibrio', 'Equilíbrio', '#4f7a3a', [
    { name: 'Centro Vivo', desc: '+2% de defesa mágica. Enquanto o Descompasso estiver em 0, +2% de defesa.', effect: { mdefPct: 0.02 },
      mechanicRefs: ['druida:dissonance'] },
    { name: 'Visão do Pêndulo', desc: '+1,5 ponto percentual de precisão. Habilidades de Equilíbrio Sintonizadas recebem bônus adicional por SAB total (até +2 pontos percentuais).', effect: { accuracyPct: 0.015 },
      scaling: [{ attribute: 'wis', label: 'SAB', role: 'principal', description: 'Aumenta a precisão de habilidades de Equilíbrio Sintonizadas (até +2pp).' }] },
    { name: 'Corpo de Carvalho', desc: '+18 Vida Máxima. Com Descompasso 2 ou mais, +2% de defesa. +2% de eficiência de cura.', effect: { maxHpFlat: 18, healingPowerPct: 0.02 },
      mechanicRefs: ['druida:dissonance'] },
    { name: 'Compasso Natural', desc: '-3% de recarga somente para habilidades de Equilíbrio.', effect: { cooldownReductionPct: 0.03 } },
    { name: 'Vinhas Errantes', desc: 'Habilidade ativa: 1,25x de MATK. Se Sintonizada, 1,35x e cura 4% da Vida Base. Se Reequilibrada, 1,55x e cura 7%. Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:dissonance', 'druida:reequilibrium'],
      ability: { name: 'Vinhas Errantes', desc: '1,25x MATK. Primavera: 1,35x e cura 4% da Vida Base. Reequilibrada: 1,55x e cura 7%.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'bigHit', dmgMult: 1.25, dmgMultAligned: 1.35, dmgMultRebalanced: 1.55, healPctAligned: 0.04, healPctRebalanced: 0.07, druidSeason: 'spring', druidPath: 'balance' } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano do golpe.' }] },
    { name: 'Casca Equilibrada', desc: '+2% de defesa. Enquanto o Descompasso estiver em 0, recebe 2% a menos de dano direto.', effect: { defPct: 0.02 },
      mechanicRefs: ['druida:dissonance'] },
    { name: 'Descompasso', desc: 'Ativa o recurso Descompasso (0 a 3). Ataque básico e habilidades sazonais usadas fora de sua Estação geram +1; uma habilidade sazonal Sintonizada reduz -1 quando está em 1 ou 2 (trava em 3, aguardando o Reequilíbrio).', effect: {},
      mechanicRefs: ['druida:dissonance'] },
    { name: 'Força do Pêndulo', desc: '+2% de dano mágico direto. Uma habilidade Reequilibrada recebe +3% de dano final adicional (não afeta cura).', effect: { magicDmgPct: 0.02 },
      mechanicRefs: ['druida:reequilibrium'] },
    { name: 'Reequilíbrio', desc: 'Ativa a transformação: com Descompasso em 3, a próxima habilidade de Equilíbrio usada Sintonizada vira Reequilibrada e consome os 3 Descompassos.', effect: {},
      mechanicRefs: ['druida:reequilibrium', 'druida:dissonance'] },
    { name: 'Sol Indomável', desc: 'Habilidade ativa: 1,60x de MATK (1,75x se Sintonizada, 2,00x com +12% de penetração de MDEF se Reequilibrada). Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:dissonance', 'druida:reequilibrium'],
      ability: { name: 'Sol Indomável', desc: '1,60x MATK. Verão: 1,75x. Reequilibrada: 2,00x com +12% de penetração de MDEF.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'bigHit', dmgMult: 1.60, dmgMultAligned: 1.75, dmgMultRebalanced: 2.00, mdefPenPct: 0, druidSeason: 'summer', druidPath: 'balance' } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano do golpe.' }] },
    { name: 'Folhas de Navalha', desc: 'Habilidade ativa: dois golpes de 0,60x de MATK (0,66x se Sintonizada; um terceiro golpe de 0,45x se Reequilibrada). Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:dissonance', 'druida:reequilibrium'],
      ability: { name: 'Folhas de Navalha', desc: 'Dois golpes de 0,60x MATK. Outono: 0,66x cada. Reequilibrada: um terceiro golpe de 0,45x.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'multiHit', hitCount: 2, hitDmgMults: [0.60, 0.60], hitDmgMultsAligned: [0.66, 0.66], hitCountRebalanced: 3, hitDmgMultsRebalanced: [0.66, 0.66, 0.45], druidSeason: 'autumn', druidPath: 'balance' } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano de cada golpe.' }] },
    { name: 'Centro Imóvel', desc: '+2% de defesa mágica. Com Descompasso em 3 (Reequilíbrio pronto), tenacidade adicional.', effect: { mdefPct: 0.02 },
      mechanicRefs: ['druida:dissonance', 'druida:reequilibrium'] },
    { name: 'Raízes do Inverno', desc: 'Habilidade ativa: 1,20x de MATK. Se Sintonizada, 1,30x e a próxima ação direta inimiga causa 6% menos dano. Se Reequilibrada, 1,45x e -12%. Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:dissonance', 'druida:reequilibrium'],
      ability: { name: 'Raízes do Inverno', desc: '1,20x MATK. Inverno: 1,30x e -6% na próxima ação direta inimiga. Reequilibrada: 1,45x e -12%.',
        cooldown: 4, condition: { type: 'always' },
        effect: { kind: 'bigHit', dmgMult: 1.20, dmgMultAligned: 1.30, dmgMultRebalanced: 1.45, druidPostCastDmgReductionPct: 0, druidPostCastDmgReductionPctAligned: 0.06, druidPostCastDmgReductionPctRebalanced: 0.12, druidSeason: 'winter', druidPath: 'balance' } },
      scaling: [{ attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o dano do golpe.' }] },
    { name: 'Eterno Retorno', desc: 'Habilidade ativa (cycle, requer Renovo ou Descompasso 3): resolve quatro manifestações sazonais em uma ação (Primavera cura, e a sequência aplica -MDEF inimiga e -dano na próxima ação inimiga). Com os dois recursos ao mesmo tempo, vira Ciclo Absoluto, muito mais forte. Recarga de 9 ciclos.',
      mechanicRefs: ['druida:renewal', 'druida:dissonance'],
      ability: { name: 'Eterno Retorno', desc: 'Quatro impactos (Primavera/Verão/Outono/Inverno) somando 1,80x de MATK; a Primavera cura 4% da Vida Base. Com Renovo E Descompasso 3: Ciclo Absoluto, 2,25x total e cura 7%.',
        cooldown: 9, condition: { type: 'any', conditions: [{ type: 'resourceAtLeast', resource: 'renewal', value: 1 }, { type: 'resourceAtLeast', resource: 'dissonance', value: 3 }] },
        effect: { kind: 'multiHit', hitCount: 4, hitDmgMults: [0.35, 0.55, 0.45, 0.45], hitDmgMultsAbsolute: [0.45, 0.70, 0.55, 0.55], healPct: 0.04, healPctAbsolute: 0.07, druidSeason: 'cycle', druidPath: 'balance', druidEternalReturn: true } },
      scaling: [{ label: 'Renovo/Descompasso', role: 'mecanica', description: 'Consome o(s) recurso(s) disponível(is) no início; com os dois juntos, ativa o Ciclo Absoluto.' }] },
    { name: 'Lei do Retorno', desc: 'Quando uma habilidade Reequilibrada resolve, repara a Sintonia da Estação mais antiga deste Ano que ainda não foi sintonizada (nunca uma Estação futura) — no máximo uma reparação por Reequilíbrio.', effect: {},
      mechanicRefs: ['druida:reequilibrium', 'druida:attunement'] },
  ]),
];
...
const warlockScale = (mechanic: string, attribute: AttributeKey = 'int'): ScalingEntry[] => [
  { attribute, label: attribute === 'int' ? 'INT' : attribute.toUpperCase(), role: 'principal', description: attribute === 'int' ? 'Aumenta o MATK usado por esta habilidade.' : 'Interage com a defesa e a resistência do Bruxo.' },
  { label: mechanic, role: 'mecanica', description: `O valor de ${mechanic} é lido no início do cast.` },
];
const warlockFixedScale = (mechanic: string): ScalingEntry[] => [
  { label: mechanic, role: 'mecanica', description: 'Não escala diretamente com nenhum atributo.' },
  { attribute: 'int', label: 'INT', role: 'principal', description: 'Aumenta o MATK usado por esta habilidade.' },
];
SKILL_TREES.bruxo = [
  buildPath('bruxo', 'maldicao', 'NOME PROIBIDO', '#4a2a5a', [
    { name: 'Olhar por Trás do Véu', desc: '+1,5 ponto percentual de Precisão. Contra um inimigo Vinculado, +2 pontos percentuais adicionais.', effect: { accuracyPct: 0.015 }, mechanicRefs: ['bruxo:binding'], scaling: warlockScale('Vínculo', 'dex') },
    { name: 'Caligrafia Profana', desc: '+2% de dano mágico direto de Nome Proibido. Com Nome Verdadeiro ativo, +2% adicionais.', effect: { magicDmgPct: 0.02 }, mechanicRefs: ['bruxo:true-name'], scaling: warlockScale('Nome Verdadeiro') },
    { name: 'Pele do Segredo', desc: '+2% de MDEF. Enquanto o inimigo estiver Vinculado, +2% de DEF adicional.', effect: { mdefPct: 0.02 }, mechanicRefs: ['bruxo:binding'], scaling: warlockScale('Vínculo', 'wis') },
    ...
    { name: 'Selo do Nome', desc: 'Causa 1,25x MATK, gera 1 Dívida e aplica Vínculo ao acertar.', mechanicRefs: ['bruxo:debt','bruxo:binding'], scaling: warlockScale('Dívida'), ability: { name: 'Selo do Nome', desc: '1,25x MATK; gera Dívida e aplica Vínculo.', cooldown: 3, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.25, warlockPath: 'maldicao', warlockDebtGain: 1, warlockBindOnHit: true } } },
    { name: 'Pronúncia Exata', desc: '+3% de Dano Crítico. Contra um inimigo com Nome Verdadeiro, +2 pontos percentuais de Crítico adicionais.', effect: { critDmgPct: 0.03 }, mechanicRefs: ['bruxo:true-name'], scaling: warlockScale('Nome Verdadeiro','luk') },
    ...
    { name: 'Anatomia da Alma', desc: 'Contra um inimigo Vinculado, +4% de penetração de MDEF, com INT concedendo até +7% no total.', mechanicRefs: ['bruxo:binding'], scaling: warlockScale('Vínculo') },
    ...
    { name: 'Exigir Tributo', desc: 'Requer Nome Verdadeiro ativo e 1 ou mais de Dívida. Consome 3 Fragmentos, causa 1,55x MATK e reduz a Dívida em 2, mesmo se errar.', mechanicRefs: ['bruxo:true-name','bruxo:debt'], scaling: warlockScale('Dívida'), ability: { name: 'Exigir Tributo', desc: 'Consome Nome Verdadeiro; 1,55x MATK e paga 2 Dívidas.', cooldown: 4, condition: { type: 'all', conditions: [{ type: 'resourceAtLeast', resource: 'debt', value: 1 }, { type: 'stateActive', state: 'trueName' }] }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.55, warlockPath: 'maldicao', warlockDebtPay: 2, warlockConsumeTrueName: true, warlockBindOnHit: true } } },
    { name: 'Mandamento de Fraqueza', desc: 'Causa 1,45x MATK, gera Dívida e, ao acertar, reduz em 10% o dano direto da próxima ação inimiga.', mechanicRefs: ['bruxo:deadline','bruxo:binding'], scaling: warlockScale('Prazo Final'), ability: { name: 'Mandamento de Fraqueza', desc: '1,45x MATK e Mandamento na próxima ação inimiga.', cooldown: 5, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.45, warlockPath: 'maldicao', warlockDebtGain: 1, warlockBindOnHit: true, warlockNextEnemyDmgReductionPct: 0.10 } } },
    ...
    { name: 'Palavra Proibida', desc: 'Requer e consome Nome Verdadeiro. Causa 1,90x MATK, gera Dívida e aplica Silêncio por 1 ciclo ao acertar.', mechanicRefs: ['bruxo:true-name','bruxo:debt'], scaling: warlockScale('Nome Verdadeiro'), ability: { name: 'Palavra Proibida', desc: '1,90x MATK; silencia por 1 ciclo.', cooldown: 7, condition: { type: 'stateActive', state: 'trueName' }, effect: { kind: 'crowdControl', dmgType: 'magical', dmgMult: 1.90, cc: 'silence', ccRounds: 1, warlockPath: 'maldicao', warlockDebtGain: 1, warlockConsumeTrueName: true, warlockBindOnHit: true } } },
    { name: 'Apagar o Nome', desc: 'Requer Nome Verdadeiro ativo e 1 ou mais de Dívida. Consome 3 Fragmentos, causa 2,55x MATK com 15% de penetração de MDEF e paga 3 Dívidas mesmo se errar.', mechanicRefs: ['bruxo:true-name','bruxo:debt'], scaling: warlockScale('Dívida'), ability: { name: 'Apagar o Nome', desc: '2,55x MATK e +15% penetração; paga 3 Dívidas.', cooldown: 9, condition: { type: 'all', conditions: [{ type: 'resourceAtLeast', resource: 'debt', value: 1 }, { type: 'stateActive', state: 'trueName' }] }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 2.55, warlockMdefPenPct: 0.15, warlockPath: 'maldicao', warlockDebtPay: 3, warlockConsumeTrueName: true, warlockBindOnHit: true } } },
  ...
  buildPath('bruxo', 'pacto', 'NEGOCIAÇÃO', '#5b5f6a', [
    ...
    { name: 'Sangue Frio', desc: '+2% de MDEF. Com 4 ou mais de Dívida, +2% de MDEF adicionais.', effect: { mdefPct: 0.02 }, mechanicRefs: ['bruxo:debt'], scaling: warlockScale('Dívida','wis') },
    { name: 'Garantia Oculta', desc: '+2 pontos percentuais de Tenacidade.', effect: {}, mechanicRefs: ['bruxo:credit'], scaling: warlockScale('Tenacidade','vit') },
    ...
    { name: 'Escudo do Fiador', desc: 'Causa 1,20x MATK, gera Dívida e cria uma barreira de 7% da Vida Máxima depois de resolver.', mechanicRefs: ['bruxo:credit','bruxo:debt'], scaling: warlockScale('Crédito'), ability: { name: 'Escudo do Fiador', desc: '1,20x MATK e barreira de 7%.', cooldown: 4, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.20, warlockPath: 'pacto', warlockDebtGain: 1, warlockBarrierPct: 0.07, scalesWithBarrierPower: true } } },
    { name: 'Juros Sob Controle', desc: '+2% de DEF. Enquanto possuir Crédito, +2% de MDEF adicional.', effect: { defPct: 0.02 }, mechanicRefs: ['bruxo:credit'], scaling: warlockScale('Crédito','wis') },
    ...
    { name: 'Cláusula de Proteção', desc: '+6 de Vida Máxima. Barreiras de Negociação recebem bônus adicional por SAB, até +4%.', effect: { maxHpFlat: 6 }, mechanicRefs: ['bruxo:credit'], scaling: warlockScale('Barreiras','wis') },
    ...
    { name: 'Renegociar Termos', desc: 'Requer 2 ou mais de Dívida. Causa 1,25x MATK, paga 2 Dívidas, gera Crédito e reduz o dano direto recebido em 8% por 2 ciclos.', mechanicRefs: ['bruxo:credit','bruxo:debt'], scaling: warlockScale('Crédito'), ability: { name: 'Renegociar Termos', desc: '1,25x MATK; paga 2 Dívidas e prepara proteção.', cooldown: 5, condition: { type: 'resourceAtLeast', resource: 'debt', value: 2 }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.25, warlockPath: 'pacto', warlockDebtPay: 2, warlockGrantCredits: 1, warlockNextEnemyDmgReductionPct: 0.08 } } },
    { name: 'Cláusula de Sangue', desc: 'Requer 3 ou mais de Dívida. Paga 6% da Vida Máxima em HP no início, causa 1,70x MATK, paga 3 Dívidas e gera Crédito.', mechanicRefs: ['bruxo:credit','bruxo:debt'], scaling: warlockScale('Dívida'), ability: { name: 'Cláusula de Sangue', desc: 'Paga 6% HP; 1,70x MATK e quita 3 Dívidas.', cooldown: 6, condition: { type: 'resourceAtLeast', resource: 'debt', value: 3 }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.70, warlockPath: 'pacto', warlockDebtPay: 3, warlockGrantCredits: 1, warlockSelfHpCostPct: 0.06 } } },
    { name: 'Reserva Contratual', desc: '+8 Vida Máxima. Depois de pagar 1 ou mais de Dívida, recebe -4% de dano direto até a próxima ação inimiga.', effect: { maxHpFlat: 8 }, mechanicRefs: ['bruxo:debt'], scaling: warlockScale('Dívida','vit') },
    { name: 'Empréstimo Abissal', desc: 'Causa 2,00x MATK, gera Dívida e cria uma barreira de 10% da Vida Máxima.', mechanicRefs: ['bruxo:credit','bruxo:debt'], scaling: warlockScale('Crédito'), ability: { name: 'Empréstimo Abissal', desc: '2,00x MATK e barreira de 10%.', cooldown: 7, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 2.00, warlockPath: 'pacto', warlockDebtGain: 1, warlockBarrierPct: 0.10, scalesWithBarrierPower: true } } },
    { name: 'Contrato Irrevogável', desc: 'Requer 5 ou mais de Dívida. Causa 2,05x MATK usando a Dívida pré-pagamento; depois fixa a Dívida em 1 e concede 2 Créditos.', mechanicRefs: ['bruxo:debt','bruxo:credit'], scaling: warlockScale('Dívida'), ability: { name: 'Contrato Irrevogável', desc: '2,05x MATK; fixa Dívida em 1 e concede 2 Créditos.', cooldown: 10, condition: { type: 'resourceAtLeast', resource: 'debt', value: 5 }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 2.05, warlockPath: 'pacto', warlockDebtSetAfter: 1, warlockGrantCredits: 2 } } },
  ...
  buildPath('bruxo', 'corrupcao', 'TRANSGRESSÃO', '#c89a2e', [
    ...
    { name: 'Olhos Marcados', desc: '+1,5 ponto percentual de Precisão. Com 2 ou mais Estigmas, +2 pontos percentuais adicionais.', effect: { accuracyPct: 0.015 }, mechanicRefs: ['bruxo:scars'], scaling: warlockScale('Estigmas','dex') },
    ...
    { name: 'Fenda no Contrato', desc: 'Causa 1,35x MATK, gera Dívida e recebe +5% de dano direto se sua Dívida for 5 ou mais.', mechanicRefs: ['bruxo:debt','bruxo:scars'], scaling: warlockScale('Dívida'), ability: { name: 'Fenda no Contrato', desc: '1,35x MATK; gera Dívida e amplifica com dívida alta.', cooldown: 3, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.35, warlockPath: 'corrupcao', warlockDebtGain: 1 } } },
    { name: 'Vaso Imperfeito', desc: '+8 Vida Máxima.', effect: { maxHpFlat: 8 }, mechanicRefs: ['bruxo:scars'], scaling: warlockScale('Vida Máxima','vit') },
    { name: 'Estigmas', desc: 'Desbloqueia Estigmas de 0 a 3: toda Cobrança real de 5% ou mais da Vida Máxima gera 1 Estigma, que concede +3% de dano de Transgressão e -2% de MDEF.', mechanicRefs: ['bruxo:scars'], scaling: warlockFixedScale('Estigmas') },
    { name: 'Dor Familiar', desc: '+3% de Dano Crítico. Com 3 Estigmas, +3% adicionais.', effect: { critDmgPct: 0.03 }, mechanicRefs: ['bruxo:scars'], scaling: warlockScale('Estigmas','luk') },
    ...
    { name: 'Cobrança Antecipada', desc: 'Requer 4 ou mais de Dívida. Causa 1,60x MATK, depois cobra 7% da Vida Máxima e paga 2 Dívidas, mesmo se errar.', mechanicRefs: ['bruxo:debt','bruxo:scars'], scaling: warlockScale('Cobrança'), ability: { name: 'Cobrança Antecipada', desc: '1,60x MATK; cobrança de 7% e quitação de 2 Dívidas.', cooldown: 5, condition: { type: 'resourceAtLeast', resource: 'debt', value: 4 }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.60, warlockPath: 'corrupcao', warlockDebtPay: 2, warlockEarlyCollectionPct: 0.07 } } },
    { name: 'Carne como Pergaminho', desc: 'Causa 1,75x MATK, gera Dívida e recebe +3% de dano direto por Estigma ativo no início do cast.', mechanicRefs: ['bruxo:scars','bruxo:debt'], scaling: warlockScale('Estigmas'), ability: { name: 'Carne como Pergaminho', desc: '1,75x MATK com bônus por Estigma.', cooldown: 5, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.75, warlockPath: 'corrupcao', warlockDebtGain: 1, warlockDmgMultPerScar: 0.03 } } },
    { name: 'Vozes sob a Pele', desc: '+2 pontos percentuais de Tenacidade. Depois de sofrer uma Cobrança, +3 pontos percentuais adicionais até a próxima ação inimiga.', effect: {}, mechanicRefs: ['bruxo:scars'], scaling: warlockScale('Estigmas','vit') },
    { name: 'Devorar Cicatrizes', desc: 'Requer 1 ou mais Estigma. Consome todos no início do cast, causando 1,50x MATK +0,30x por Estigma consumido, e gera Dívida.', mechanicRefs: ['bruxo:scars','bruxo:debt'], scaling: warlockScale('Estigmas'), ability: { name: 'Devorar Cicatrizes', desc: 'Consome Estigmas para dano crescente.', cooldown: 7, condition: { type: 'resourceAtLeast', resource: 'scars', value: 1 }, effect: { kind: 'bigHit', dmgType: 'magical', dmgMult: 1.50, warlockPath: 'corrupcao', warlockDebtGain: 1, warlockConsumeScars: true, warlockDmgMultPerScar: 0.30 } } },
    { name: 'A Voz que Responde', desc: 'Requer 3 Estigmas, 5 ou mais de Dívida e vida suficiente para a Cobrança. Consome os Estigmas, desfere 3 impactos de 0,85x MATK, cobra 12% da Vida Máxima, fixa a Dívida em 3 e ecoa o HP pago de volta no inimigo.', mechanicRefs: ['bruxo:scars','bruxo:debt'], scaling: warlockScale('Cobrança'), ability: { name: 'A Voz que Responde', desc: '3 impactos de 0,85x, Cobrança Forçada de 12% e Eco do Preço.', cooldown: 10, condition: { type: 'all', conditions: [{ type: 'resourceAtLeast', resource: 'scars', value: 3 }, { type: 'resourceAtLeast', resource: 'debt', value: 5 }] }, effect: { kind: 'multiHit', hitCount: 3, dmgMultPerHit: 0.85, dmgType: 'magical', warlockPath: 'corrupcao', warlockConsumeScars: true, warlockForcedCollectionPct: 0.12, warlockDebtSetAfter: 3, warlockCollectionEchoPct: 1 } } },
...
SKILL_TREES.feiticeiro = [
  buildPath('feiticeiro', 'explosao', 'RUPTURA', '#a03fb8', [
    { name:'Pressão Interior', desc:'+2% de dano mágico direto de Ruptura. Com 4 ou mais de Pulso, +2% adicionais.', effect:{ magicDmgPct:0.02 }, scaling:sorcererScale('Pulso Inato'), mechanicRefs:['feiticeiro:pulse'] },
    { name:'Alvo de Impacto', desc:'+1,5 ponto percentual de Precisão. Magia Intensificada recebe +1,5 pontos percentuais adicionais.', effect:{ accuracyPct:0.015 }, scaling:sorcererScale('Precisão'), mechanicRefs:['feiticeiro:awakened'] },
    { name:'Núcleo Supercrítico', desc:'+3% de Dano Crítico. Intensificada recebe +3% adicionais.', effect:{ critDmgPct:0.03 }, scaling:sorcererScale('Crítico'), mechanicRefs:['feiticeiro:awakened'] },
    { name:'Ritmo de Ruptura', desc:'-3% de Recarga somente para habilidades de Ruptura.', effect:{ cooldownReductionPct:0.03 }, scaling:sorcererFixed('Recarga') },
    { name:'Impacto de Origem', desc:'Causa 1,35x MATK. Enquanto Desperta, esta magia se torna Intensificada.', ...sActive('Impacto de Origem','1,35x MATK.',3,{kind:'bigHit',dmgType:'magical',dmgMult:1.35,sorcererPath:'rupture',sorcererAwakenedMode:'intensified',sorcererFractureGain:1}), scaling:sorcererScale('Fraturas') },
    { name:'Tensão Crescente', desc:'+1,5 ponto percentual de Crítico. Com 4 ou mais de Pulso, +1,5 ponto percentual adicional.', effect:{ critPct:0.015 }, scaling:sorcererScale('Pulso Inato') },
    { name:'Fratura Arcana', desc:'Desbloqueia Fraturas de 0 a 3; uma Ruptura Desperta que acertar gera +1.', effect:{}, mechanicRefs:['feiticeiro:fracture'], scaling:sorcererFixed('Fraturas') },
    { name:'Matriz Trincada', desc:'+2% de dano direto de Ruptura. Com 3 Fraturas, +3%.', effect:{ magicDmgPct:0.02 }, mechanicRefs:['feiticeiro:fracture'], scaling:sorcererScale('Fraturas') },
    ...
    { name:'Compressão Violenta', desc:'Causa 1,55x MATK +0,08x por Fratura ativa no início do cast.', ...sActive('Compressão Violenta','1,55x MATK mais Fraturas.',4,{kind:'bigHit',dmgType:'magical',dmgMult:1.55,sorcererPath:'rupture',sorcererAwakenedMode:'intensified',sorcererThirdHitBonusPerFracture:0.08}), scaling:sorcererScale('Fraturas') },
    { name:'Ruptura Gêmea', desc:'Dois impactos independentes de 0,80x MATK.', ...sActive('Ruptura Gêmea','Dois impactos de 0,80x MATK.',5,{kind:'multiHit',dmgType:'magical',hitCount:2,dmgMultPerHit:0.80,sorcererPath:'rupture',sorcererAwakenedMode:'intensified'}), scaling:sorcererScale('Multi-hit') },
    { name:'Limiar do Colapso', desc:'+4% de Dano Crítico. Contra um inimigo com 3 Fraturas, +2 pontos percentuais de Crítico adicionais.', effect:{ critDmgPct:0.04 }, mechanicRefs:['feiticeiro:fracture'] },
    { name:'Ponto de Colapso', desc:'Consome as Fraturas ativas; causa 1,55x MATK +0,25x por Fratura consumida.', ...sActive('Ponto de Colapso','Consome todas as Fraturas.',7,{kind:'bigHit',dmgType:'magical',dmgMult:1.55,sorcererPath:'rupture',sorcererFractureConsume:3,sorcererAwakenedMode:'intensified'},{ type:'enemyStacksAtLeast', stackId:'fracture', stacks:1 }), scaling:sorcererScale('Fraturas') },
    { name:'Supernova Interior', desc:'Três impactos de 0,70x MATK cada, com o terceiro recebendo +0,12x por Fratura consumida. Consome todas as Fraturas.', ...sActive('Supernova Interior','Três impactos de Ruptura.',10,{kind:'multiHit',dmgType:'magical',hitCount:3,dmgMultPerHit:0.70,hitDmgMults:[0.70,0.70,0.70],sorcererPath:'rupture',sorcererFractureConsume:3,sorcererThirdHitBonusPerFracture:0.12,sorcererAwakenedMode:'intensified'}), scaling:sorcererScale('Fraturas') },
  ...
  buildPath('feiticeiro', 'sobrecarga', 'REVERBERAÇÃO', '#c89a2e', [
    ...
    { name:'Reflexo Duplo', desc:'+1,5 ponto percentual de Crítico. Um Eco recebe +1 ponto percentual adicional.', effect:{ critPct:0.015 }, mechanicRefs:['feiticeiro:resonance'] },
    { name:'Frequência Estável', desc:'+1,5 ponto percentual de Precisão. Um Eco recebe +1,5 ponto percentual adicional.', effect:{ accuracyPct:0.015 }, mechanicRefs:['feiticeiro:resonance'] },
    { name:'Ritmo Recorrente', desc:'-3% de Recarga somente para Reverberação.', effect:{ cooldownReductionPct:0.03 } },
    { name:'Pulso Bifurcado', desc:'Dois impactos de 0,68x MATK; cada Eco gerado causa 0,272x.', ...sActive('Pulso Bifurcado','Dois impactos de 0,68x MATK.',3,{kind:'multiHit',dmgType:'magical',hitCount:2,dmgMultPerHit:0.68,sorcererPath:'reverberation',sorcererAwakenedMode:'refracted',sorcererEchoPotency:0.40}), scaling:sorcererScale('Eco Arcano') },
    { name:'Aceleração Harmônica', desc:'+1,5% de Velocidade de ação. Com Ressonância ativa, +1% adicional.', effect:{}, mechanicRefs:['feiticeiro:resonance'] },
    { name:'Ressonância', desc:'Desbloqueia Ressonância de 0 a 2. Uma magia Desperta gera +1; a próxima magia normal consome 1 e concede +2 de Pulso.', effect:{}, mechanicRefs:['feiticeiro:resonance'] },
    { name:'Câmara de Eco', desc:'+3% de Dano Crítico. Um Eco recebe +3% adicional.', effect:{ critDmgPct:0.03 }, mechanicRefs:['feiticeiro:resonance'] },
    { name:'Segunda Onda', desc:'Se o impacto original critar, os Ecos gerados recebem +6 pontos percentuais de Crítico.', effect:{}, mechanicRefs:['feiticeiro:resonance'] },
    { name:'Eco Instável', desc:'Causa 1,50x MATK; com Pulso entre 4 e 5, +6% de dano adicional.', ...sActive('Eco Instável','1,50x MATK.',4,{kind:'bigHit',dmgType:'magical',dmgMult:1.50,sorcererPath:'reverberation',sorcererAwakenedMode:'refracted',sorcererEchoPotency:0.40}), scaling:sorcererScale('Pulso Inato') },
    { name:'Cascata Arcana', desc:'Três impactos de 0,55x MATK cada, cada um gerando um Eco.', ...sActive('Cascata Arcana','Três impactos de 0,55x MATK.',5,{kind:'multiHit',dmgType:'magical',hitCount:3,dmgMultPerHit:0.55,sorcererPath:'reverberation',sorcererAwakenedMode:'refracted',sorcererEchoPotency:0.40}), scaling:sorcererScale('Eco Arcano') },
    { name:'Memória Harmônica', desc:'+2% de dano de Reverberação. Com Ressonância ativa, +2% de Dano Crítico adicional.', effect:{ magicDmgPct:0.02 }, mechanicRefs:['feiticeiro:resonance'] },
    { name:'Recorrência', desc:'Causa 1,75x MATK. Ao acertar, reduz em 1 ciclo a recarga restante de outra habilidade equipada.', ...sActive('Recorrência','1,75x MATK e reduz o maior cooldown.',7,{kind:'bigHit',dmgType:'magical',dmgMult:1.75,sorcererPath:'reverberation',sorcererCooldownCutOnHit:true}), scaling:sorcererScale('Recarga') },
    { name:'Sobrecarga Viva', desc:'Quatro impactos de 0,50x MATK cada, com Ecos somando +0,80x de dano total.', ...sActive('Sobrecarga Viva','Quatro impactos de 0,50x MATK.',9,{kind:'multiHit',dmgType:'magical',hitCount:4,dmgMultPerHit:0.50,sorcererPath:'reverberation',sorcererAwakenedMode:'refracted',sorcererEchoPotency:0.40}), scaling:sorcererScale('Eco Arcano') },
    { name:'Perpetuum Arcano', desc:'Quando o Pulso ultrapassa 6 com um excedente de 2 ou mais, gera +1 de Ressonância.', effect:{}, mechanicRefs:['feiticeiro:pulse','feiticeiro:resonance'] },
  ]),
  buildPath('feiticeiro', 'dominio', 'MOLDAGEM', '#c1502e', [
    { name:'Geometria Instintiva', desc:'+1,5 ponto percentual de Precisão. Com 4 ou mais de Pulso, +1 ponto percentual adicional.', effect:{ accuracyPct:0.015 } },
    { name:'Canal Estreito', desc:'+2% dano mágico direto de Moldagem.', effect:{ magicDmgPct:0.02 } },
    { name:'Pele de Mana', desc:'+2% de MDEF. Com 2 de Controle, +2% adicionais.', effect:{ mdefPct:0.02 }, mechanicRefs:['feiticeiro:control'] },
    { name:'Ritmo Preciso', desc:'-3% de Recarga somente para Moldagem.', effect:{ cooldownReductionPct:0.03 } },
    { name:'Vetor Forçado', desc:'Causa 1,30x MATK e +4 pontos percentuais de Precisão.', ...sActive('Vetor Forçado','1,30x MATK e +4pp Precisão.',3,{kind:'bigHit',dmgType:'magical',dmgMult:1.30,sorcererPath:'shaping',sorcererAccuracyBonusPct:0.04,sorcererAwakenedMode:'molded'}), scaling:sorcererScale('Controle') },
    { name:'Mira Arcana', desc:'+1,5 ponto percentual de Precisão. Com Controle ativo, +1,5 ponto percentual adicional.', effect:{ accuracyPct:0.015 }, mechanicRefs:['feiticeiro:control'] },
    { name:'Controle Arcano', desc:'Desbloqueia Controle de 0 a 2; Desperta gera +1; cada carga concede +2 pontos percentuais de Precisão e +2% de penetração.', effect:{}, mechanicRefs:['feiticeiro:control'] },
    { name:'Estrutura Densa', desc:'+3% de penetração de MDEF. Com 2 de Controle, +2% adicional.', effect:{}, mechanicRefs:['feiticeiro:control'] },
    { name:'Correção Instantânea', desc:'Uma vez por inimigo, uma falha total com Controle ativo rerrola o primeiro impacto com +10 pontos percentuais de Precisão.', effect:{}, mechanicRefs:['feiticeiro:control'] },
    { name:'Campo de Distorção', desc:'Causa 1,30x MATK; ao acertar, prepara -8% de dano na próxima ação inimiga (-12% e +4 pontos percentuais de Precisão com Controle).', ...sActive('Campo de Distorção','1,30x MATK e proteção contra a próxima ação.',4,{kind:'bigHit',dmgType:'magical',dmgMult:1.30,sorcererPath:'shaping',sorcererControlConsume:1,sorcererAccuracyBonusPct:0.04,sorcererEnemyDmgReductionPct:0.08,sorcererEnemyDmgReductionRounds:1,sorcererAwakenedMode:'molded'}), scaling:sorcererScale('Controle') },
    { name:'Corte de Realidade', desc:'Causa 1,75x MATK com +10% de penetração de MDEF.', ...sActive('Corte de Realidade','1,75x MATK com penetração.',5,{kind:'bigHit',dmgType:'magical',dmgMult:1.75,sorcererPath:'shaping',sorcererMdefPenPct:0.10}), scaling:sorcererScale('Penetração') },
    { name:'Constante Impossível', desc:'+2% de MDEF. Com 2 de Controle, +3% de dano de Moldagem adicional.', effect:{ mdefPct:0.02 }, mechanicRefs:['feiticeiro:control'] },
    { name:'Forma Impossível', desc:'Três impactos de 0,60x MATK; consome 1 Controle e concede +8 pontos percentuais de Precisão; o terceiro impacto recebe +15% de penetração.', ...sActive('Forma Impossível','Três impactos de 0,60x MATK.',7,{kind:'multiHit',dmgType:'magical',hitCount:3,dmgMultPerHit:0.60,sorcererPath:'shaping',sorcererControlConsume:1,sorcererAccuracyBonusPct:0.08,sorcererThirdHitPenPct:0.15,sorcererAwakenedMode:'molded'}), scaling:sorcererScale('Controle') },
    { name:'Forma Absoluta', desc:'Consome 2 Controle; causa 2,35x MATK com +15 pontos percentuais de Precisão e +20% de penetração.', ...sActive('Forma Absoluta','2,35x MATK com precisão e penetração máximas.',9,{kind:'bigHit',dmgType:'magical',dmgMult:2.35,sorcererPath:'shaping',sorcererControlConsume:2,sorcererAccuracyBonusPct:0.15,sorcererMdefPenPct:0.20,sorcererAwakenedMode:'molded'},{ type:'resourceAtLeast', resource:'control', value:2 }), scaling:sorcererScale('Controle') },
...
SKILL_TREES.bardo = [
  buildPath('bardo','cancao-guerra','MARCHA DE GUERRA','#c9663c',[
    bNode('Pulso Marcial','Habilidades ofensivas de Marcha causam +2% de dano direto. Com Acento ativo, DES concede até +3% adicionais.',{magicDmgPct:0.02},['bardo:accent'],bardScale('Acento','dex')),
    bNode('Mão Certeira','+1,5 ponto percentual de Precisão. Habilidades Acentuadas recebem +2 pontos percentuais adicionais.',{accuracyPct:0.015},['bardo:accent'],bardScale('Precisão','dex')),
    bNode('Voz de Batalha','+3% de Dano Crítico. Fortíssimo concede +3% adicionais.',{critDmgPct:0.03},['bardo:fortissimo'],bardScale('Crítico','luk')),
    bNode('Compasso de Guerra','-3% de Recarga somente para Marcha de Guerra.',{cooldownReductionPct:0.03},['bardo:score'],bardScale('Recarga')),
    bAct('Acorde de Impacto','Causa 1,30x MATK como Nota Marcato; com Acento, adiciona 0,35x de ATK físico.',3,bVoice('marcato','march',{kind:'bigHit',dmgMult:1.30,bardAccent:true,bardAccentAtkMult:0.35,bardEncoreEligible:true})),
    bNode('Passo no Compasso','+1,5% de Velocidade de ação. Ao consumir Acento, +2% adicionais até sua próxima ação.',{},['bardo:accent'],bardScale('Acento')),
    bNode('Batida Marcada','Desbloqueia Acento: ataque básico acertado ou Frase Marcato o prepara.',{},['bardo:accent'],bardScale('Acento')),
    bNode('Cordas de Aço','+3% de Dano Crítico. O componente físico do Acento recebe bônus de DES, até +0,06x.',{critDmgPct:0.03},['bardo:accent'],bardScale('DES total','dex')),
    bNode('Sem Perder o Ritmo','Uma vez por inimigo, um erro total de Marcato com Acento ativo devolve o Acento.',{},['bardo:accent'],bardScale('Reembolso')),
    bAct('Marcha Implacável','Dois impactos mágicos de 0,63x cada; com Acento, adiciona 0,40x de ATK físico.',4,bVoice('marcato','march',{kind:'multiHit',hitCount:2,dmgMultPerHit:0.63,bardAccent:true,bardAccentAtkMult:0.40,bardEncoreEligible:true})),
    bAct('Grito em Compasso','Causa 1,50x MATK; ao acertar, +5% de Velocidade por 2 ciclos.',5,bVoice('marcato','march',{kind:'bigHit',dmgMult:1.50,bardSpeedBuffPct:0.05,bardSpeedBuffRounds:2,bardEncoreEligible:true})),
    bNode('Presença de Palco','+8 de Vida Máxima. Fortíssimo reduz em 3% o dano direto recebido.',{maxHpFlat:8},['bardo:fortissimo'],bardScale('Fortíssimo')),
    bAct('Solo de Batalha','Causa 2,00x MATK; com Acento, adiciona 0,50x de ATK físico.',7,bVoice('marcato','march',{kind:'bigHit',dmgMult:2.00,bardAccent:true,bardAccentAtkMult:0.50,bardEncoreEligible:true})),
    bAct('Concerto de Guerra','Finalização: requer e consome Ovação. Causa 1,55x MATK e 0,65x de ATK físico; com Acento, adiciona 0,20x de ATK físico.',9,bVoice('finale','march',{kind:'multiHit',hitCount:2,hitDmgMults:[1.55,0.65],bardFinale:true,bardOvationCost:1,bardAccentAtkMult:0.20,bardMagicalHitMults:[1.55],bardPhysicalHitMults:[0.65]}),{ type:'resourceAtLeast',resource:'ovation',value:1 }),
  ...
  buildPath('bardo','melodia-sombria','DISSONÂNCIA','#4a2a5a',[
    bNode('Ouvido Cruel','+1,5 ponto percentual de Precisão. Contra um inimigo com Contratempo, +2 pontos percentuais adicionais.',{accuracyPct:0.015},['bardo:countertempo'],bardScale('Precisão','dex')),
    bNode('Frequência Cortante','+4% de penetração de MDEF. Com Eco ativo, INT concede até +3% adicionais.',{},['bardo:echo'],bardScale('Eco','int')),
    bNode('Tom Incômodo','+2% de MDEF. Com Eco ativo, +2% de DEF adicional.',{mdefPct:0.02},['bardo:echo'],bardScale('Eco')),
    bNode('Compasso Quebrado','-3% de Recarga somente para Dissonância.',{cooldownReductionPct:0.03},['bardo:score'],bardScale('Recarga')),
    bAct('Nota Quebrada','Causa 1,25x MATK; ao acertar, aplica Contratempo.',3,bVoice('dissonant','dissonance',{kind:'bigHit',dmgMult:1.25,bardAppliesCountertempo:true,bardEncoreEligible:true})),
    bNode('Resistência ao Ruído','+2 pontos percentuais de Tenacidade. Com 2 Ecos, +2 pontos percentuais adicionais.',{},['bardo:echo'],bardScale('Tenacidade')),
    bNode('Eco Roubado','Desbloqueia Eco de 0 a 2, gerado por Contratempo.',{},['bardo:echo','bardo:countertempo'],bardScale('Eco')),
    bNode('Harmonia Áspera','+3% de Dano Crítico. Com 2 Ecos, +3% adicionais.',{critDmgPct:0.03},['bardo:echo'],bardScale('Crítico','luk')),
    bNode('Contratempo Perfeito','Um Contratempo aplicado durante Fora de Tom gera +1 Eco adicional, até um máximo de 2.',{},['bardo:countertempo','bardo:echo'],bardScale('Eco')),
    bAct('Trítono','Causa 1,45x MATK; com Eco disponível, consome 1 e sobe para 1,70x. Ao acertar, -6% de MDEF por 2 ciclos.',4,bVoice('dissonant','dissonance',{kind:'bigHit',dmgMult:1.45,bardEchoCost:1,bardMdefDebuffPct:0.06,bardMdefDebuffRounds:2,bardEncoreEligible:true})),
    bAct('Quebra de Compasso','Causa 1,15x MATK e aplica Contratempo; a próxima ação inimiga causa -8% de dano (-14% com 2 Ecos).',5,bVoice('dissonant','dissonance',{kind:'bigHit',dmgMult:1.15,bardAppliesCountertempo:true,bardEchoCost:2,bardNextEnemyDamageReductionPct:0.08,bardEncoreEligible:true})),
    bNode('Som que Fica','+2% de MDEF. Ao consumir Eco, +4 pontos percentuais de Tenacidade até a próxima ação.',{mdefPct:0.02},['bardo:echo'],bardScale('Eco')),
    bAct('Ressonância Partida','Requer e consome 2 Ecos. Causa 2,05x MATK com +12% de penetração (2,25x contra um inimigo Fora de Tom).',7,bVoice('dissonant','dissonance',{kind:'bigHit',dmgMult:2.05,bardEchoCost:2,bardEncoreEligible:true}),{ type:'resourceAtLeast',resource:'echo',value:2 }),
    bAct('Réquiem sem Palavras','Finalização: quatro impactos de 0,40x, 0,40x, 0,40x e 0,50x MATK; o último acerto silencia por 1 ciclo e aplica Contratempo.',10,bVoice('finale','dissonance',{kind:'multiHit',hitCount:4,hitDmgMults:[0.40,0.40,0.40,0.50],bardFinale:true,bardOvationCost:1,bardAppliesCountertempo:true,bardMagicalHitMults:[0.40,0.40,0.40,0.50]}),{ type:'resourceAtLeast',resource:'ovation',value:1 }),
    bNode('O Som Depois do Som','Consumir 2 Ecos prepara Nota Eco: a próxima magia Dissonante registra duas notas na Partitura.',{},['bardo:echo','bardo:score'],bardScale('Nota Eco')),
  ]),
  buildPath('bardo','inspiracao','IMPROVISO','#e0c060',[
    bNode('Voz Serena','+2% de MDEF. Curas ativas de Improviso recebem até +3% adicionais por SAB.',{mdefPct:0.02},['bardo:wildcard'],bardScale('Cura','wis')),
    bNode('Ouvido Absoluto','+1,5 ponto percentual de Precisão. Notas Coringa ofensivas recebem +2 pontos percentuais adicionais.',{accuracyPct:0.015},['bardo:wildcard'],bardScale('Precisão','dex')),
    bNode('Fôlego de Artista','+8 de Vida Máxima. Depois de uma Harmonia, -3% de dano direto recebido até a próxima ação.',{maxHpFlat:8},['bardo:score'],bardScale('Harmonia')),
    bNode('Respiração Musical','-3% de Recarga somente para Improviso.',{cooldownReductionPct:0.03},['bardo:score'],bardScale('Recarga')),
  bAct('Balada Restauradora','Cura 12% da Base de Cura, ampliada pelo Poder de Cura, e registra uma nota Lírica. Só pode ser usada com sua vida abaixo de 75%.',4,bVoice('lyrical','improvisation',{kind:'heal',healPct:0.12,bardSupportHealPct:0.12,bardEncoreEligible:true}),{ type:'hpBelow',pct:0.75 }),
    bNode('Clareza de Voz','+2 pontos percentuais de Tenacidade. Depois de um Refrão Lírico, +2 pontos percentuais adicionais até a próxima ação.',{},['bardo:score'],bardScale('Tenacidade')),
    bNode('Verso Livre','Desbloqueia Notas Coringa; a Harmonia passa a curar 7% em vez de 5%.',{},['bardo:wildcard'],bardScale('Coringa')),
    bNode('Respiração Profunda','+2% de MDEF. Curas de Improviso ganham até +4% adicionais por SAB.',{mdefPct:0.02},['bardo:wildcard'],bardScale('Cura','wis')),
    bNode('Ponte Musical','Um Contracanto deixa Ponte: a próxima habilidade normal de Improviso recebe +6%.',{},['bardo:score'],bardScale('Ponte')),
    bAct('Interlúdio','Cura 7% da Base de Cura e reduz em 1 ciclo o efeito negativo removível mais antigo. Nota Coringa prioriza Harmonia.',4,bVoice('wildcard','improvisation',{kind:'heal',healPct:0.07,bardSupportHealPct:0.07,bardWildcardPolicy:'harmonyFirst',bardEncoreEligible:true}),{ type:'any',conditions:[{type:'hpBelow',pct:0.85},{type:'selfDebuffed'}] }),
    bAct('Verso de Improviso','Causa 1,20x MATK. Nota Coringa prioriza Refrão.',5,bVoice('wildcard','improvisation',{kind:'bigHit',dmgMult:1.20,bardWildcardPolicy:'refrainFirst',bardEncoreEligible:true})),
    bNode('Presença Inspiradora','+2% de MDEF. Enquanto tiver Ovação guardada, +3% de DEF adicional.',{mdefPct:0.02},['bardo:ovation'],bardScale('Ovação')),
    bAct('Hino da Segunda Respiração','Cura 22% da Base de Cura (25% com Ovação) e prepara -10% de dano na próxima ação inimiga. Só pode ser usada com sua vida abaixo de 45%.',7,bVoice('lyrical','improvisation',{kind:'heal',healPct:0.22,bardSupportHealPct:0.22,bardOvationHealPct:0.25,bardNextEnemyDamageReductionPct:0.10,bardEncoreEligible:true}),{ type:'hpBelow',pct:0.45 }),
    bAct('Bis!','Finalização: requer Ovação e um Bis pronto. Repete o efeito principal da última habilidade usada, a 55% de eficiência, sem registrar nova nota nem gerar efeitos colaterais.',9,bVoice('finale','improvisation',{kind:'bigHit',bardFinale:true,bardEncore:true,bardOvationCost:1}),{ type:'all',conditions:[{type:'resourceAtLeast',resource:'ovation',value:1},{type:'stateActive',state:'encoreReady'}] }),
