...
export const SKILL_TREES: Record<ClassId, SkillPath[]> = {
  ...
  arqueiro: [
    buildPath('arqueiro', 'precisao', 'Arco Pleno', '#c89a2e', [
      ...
      { name: 'Tiro de Medição', desc: 'Disparo Preciso de 1,30x. CD 3. Gera 55 Tensão, ou 70 em Horizonte; retorna ao Horizonte; erro perde 8.', ability: { name: 'Tiro de Medição', desc: '1,30x ATK, geração especial de Tensão e retorno ao Horizonte.', cooldown: 3, condition: { type: 'always' }, effect: { kind: 'bigHit', dmgMult: 1.30, archerPath: 'precision', archerShotType: 'precise', archerTensionOverrideOnHit: 55, archerTensionOverrideAtHorizon: 70, archerDistanceShift: 3 } }, mechanicRefs: ['arqueiro:distance','arqueiro:tension'] },
      ...
      { name: 'Flecha Impossível', desc: 'Requer Tensão 100; consome tudo. 2,25x, crítico garantido, 18% pen, acelera Flechas pré-existentes. CD 9.', ability: { name: 'Flecha Impossível', desc: '2,25x ATK, crítico garantido e aceleração de voo.', cooldown: 9, condition: { type: 'resourceAtLeast', resource: 'tension', value: 100 }, effect: { kind: 'guaranteedCrit', dmgMult: 2.25, archerPath: 'precision', archerShotType: 'precise', archerRequiresTension: 100, archerTensionCost: 100, archerDefPenPct: 0.18, archerImmediateTimerReduction: 1 } }, mechanicRefs: ['arqueiro:tension','arqueiro:flight'] },
    ...
    buildPath('arqueiro', 'tiro-rapido', 'Tempestade', '#4f9d4f', [
      ...
      { name: 'Tempestade de Penas', desc: 'Requer Cadência 4/Ritmo Perfeito. Quatro impactos 0,44x; Perfeito adiciona 0,22x e acelera voos pré-existentes. CD 9.', ability: { name: 'Tempestade de Penas', desc: 'Quatro impactos e aceleração de voo.', cooldown: 9, condition: { type: 'any', conditions: [{ type: 'resourceAtLeast', resource: 'cadence', value: 4 }, { type: 'stateActive', state: 'perfectRhythm' }] }, effect: { kind: 'multiHit', hitCount: 4, hitDmgMults: [0.44,0.44,0.44,0.44], archerPath: 'rapid', archerShotType: 'volley', archerPerfectExtraRatio: 0.22, archerImmediateTimerReduction: 1 } }, mechanicRefs: ['arqueiro:cadence','arqueiro:flight'] },
...
SKILL_TREES.druida = [
  ...
  buildPath('druida', 'furia-natureza', 'Metamorfose', '#8a3a2a', [
    ...
    { name: 'Pele Ancestral', desc: '+8 Vida Máxima. Enquanto qualquer Forma estiver ativa, +2% de defesa mágica.', effect: { maxHpFlat: 8 },
      mechanicRefs: ['druida:form'] },
    { name: 'Olhos Ferais', desc: '+1,5 ponto percentual de precisão. Na Forma da Coruja, SAB total concede até +2pp adicionais.', effect: { accuracyPct: 0.015 },
      mechanicRefs: ['druida:form'],
    ...
    { name: 'Queda do Urso', desc: 'Habilidade ativa: assume a Forma do Urso. Causa 1,70x de MATK (1,90x se Sintonizada). Se Sintonizada e acertar, a redução de dano do Urso soma +2pp até a próxima ação inimiga. Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:form', 'druida:instinct'],
      ability: { name: 'Queda do Urso', desc: 'Assume a Forma do Urso. 1,70x MATK. Outono: 1,90x e +2pp de redução de dano até a próxima ação inimiga.',
        cooldown: 4, condition: { type: 'always' },
    ...
    { name: 'Espírito da Coruja', desc: '+2% de defesa mágica. Enquanto a Forma da Coruja estiver ativa, +2 pontos percentuais de precisão adicionais.', effect: { mdefPct: 0.02 },
      mechanicRefs: ['druida:form'] },
    { name: 'Olhos da Coruja', desc: 'Habilidade ativa: assume a Forma da Coruja. Causa 1,40x de MATK com 10% de penetração de MDEF (1,55x se Sintonizada, +6pp de precisão só nesse cast). Recarga de 4 ciclos.',
      mechanicRefs: ['druida:season', 'druida:form', 'druida:instinct'],
      ability: { name: 'Olhos da Coruja', desc: 'Assume a Forma da Coruja. 1,40x MATK, 10% de penetração de MDEF. Inverno: 1,55x e +6pp de precisão só nesse cast.',
        cooldown: 4, condition: { type: 'always' },
  ...
  buildPath('druida', 'equilibrio', 'Equilíbrio', '#4f7a3a', [
    ...
    { name: 'Visão do Pêndulo', desc: '+1,5 ponto percentual de precisão. Habilidades de Equilíbrio Sintonizadas recebem bônus adicional por SAB total (até +2pp).', effect: { accuracyPct: 0.015 },
