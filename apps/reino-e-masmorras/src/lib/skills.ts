...
const druidAbility = (path:string,i:number,name:string,desc:string,effect:any):AbilityDef => ({ id:`druida:${path}:${i}`, name, desc, cooldown: i===12?5:i===13?9:4, condition:{type:'always'}, effect:{...effect,druidSeason:i===4?'spring':i===9?'summer':i===10?'autumn':i===12?'winter':'cycle',druidAction:path==='cura-natural'?(i===4?'seed':i===10?'harvest':i===12?'winter':i===13?'cycle':undefined):path==='furia-natureza'?(i===13?'cycle':'form'):(i===13?'cycle':'equilibrium')} });
