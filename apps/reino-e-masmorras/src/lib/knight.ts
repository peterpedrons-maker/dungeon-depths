...
// CommandPotency — a Comando buff's own base % is scaled by
// min(CAP, SupportPowerPct * coefficient). Voz de Comando (cavaleiro:comando:0)
// raises the coefficient; the cap never moves.
...
export function commandPotency(supportPowerPct: number, coef: number): number {
  return Math.min(COMMAND_POTENCY_CAP, supportPowerPct * coef);
}
